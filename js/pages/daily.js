...
function collectAdjustments() {
  ...
  Object.keys(mem).forEach(key => {
    ...
    rows.forEach(row => {
      ...
      let anaLabel = '', growthLabel = '';
      if (hasCalc) {
        try {
          anaLabel = window.calcAnalysis(row.adsFee, row.pureRate, row.targetROI, row.roiDiff, row.clicks, row.pureProfit, row.roi).label;
          growthLabel = window.calcGrowthAnalysis(row.growthRate, row.rev, row.prevRev, row.pureRate).label;
        } catch (e) {
          anaLabel = ''; growthLabel = '';
          labelErrCount++;
        }
      }
      ...
      const pushSeg = (dateStr, rawText) => {
        ...
        seen.add(dk);
        out.push({ date: dateStr, shop, code: row.code, name: (row.name || ''), text, anaLabel, growthLabel });
      };
