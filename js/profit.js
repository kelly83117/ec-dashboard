...
async function syncToCloud(shop, allowKeys){   // allowKeys=Set → 只推選中的 key（逐項勾選）；不傳=全推
  ...
  try{
    ...
    _pendingSyncKeys.forEach(pk=>{
      ...
      if(pk.startsWith('ec_momo_e001|')){   // MO+ E001 未結算銷量 → momo_e001 collection（每賣場每來源月一 doc；未結算估算值、有對帳明細即被取代）
        const parts=pk.split('|');   // ['ec_momo_e001', shop, src]
        const eShop=parts[1], eSrc=parts[2];
        const data=momoLoadE001Doc(eShop, eSrc);   // 與預覽同源
        if(window.__cloudE001 && data && eShop && eSrc){ tasks.push({key:pk,run:()=>window.__cloudE001.setSrc(eShop,eSrc,momoFsSanitizeDeep(data))}); }
...
// ⚠ 此函式被主表大量使用（syncHeaderKpis / renderTable 商品列 / 合計列，共 6 處），
//   不是任何單一頁面專屬。2026-08-07 移除重點檢視時它就夾在該區塊正中央，差點被連坐刪除。
...
function setShop(shop,btn){
  ...
  //   而下面第一件事就是 getElementById('content-'+shop).classList.add(…)，沒有 null guard →
  //   直接拋錯。呼叫端的 try/catch 會把紅字吞掉，但此時上面兩行 querySelectorAll 已經把所有
  //   .stab / .shop-content 的 active 清光 → 畫面停在「一個分頁都沒選中」的全白，Console 什麼都不印。
  //   退回總表＝分頁列 HTML 預設就 active 的那顆，收斂到已知良好狀態；
  //   接著下面的 localStorage.setItem 會把壞掉的舊值就地覆寫，同事開一次頁面就自癒。
  //   btn 一定要一起改掉，否則會變成「內容是總表、分頁列一顆都沒亮」的半殘狀態。
  if(!document.getElementById('content-'+shop)){
    console.warn('[淨利表] 分頁「'+shop+'」已不存在，退回總表');
    shop='總表';
    btn=document.querySelector("button[onclick*=\"setShop('總表'\"]")||null;
  }
  curShop=shop;
  ...
  // show/hide KPI & upload/export when on 總表（跨賣場頁面）
...
window.addEventListener('momoE001Ready',()=>{
  try{ momoBumpMoPlusEpoch(); }catch{}   // 雲端 E001 更新 → E001 ctx 快取失效
  const shop=curMomoShop;
  if(!momoIsMoPlus(shop)) return;
  if((_momoSub[shop]||'profit')!=='profit') return;
  if(document.getElementById('momo-tbl-'+shop)) momoRenderProfitBody(shop, true);
...
// ── P6 倉租費：分頁已移除(v315)。ec_momo_rent_records 舊資料保留不清(localStorage+雲端 field 都不動、只移 UI/白名單)。倉租已由對帳單「寄倉倉租費(EC)」處理(Piece2 全額歸乙配)。──
...
//   修正跨店補貼：原本 feeRate=E/A 讓甲配吃掉乙的寄倉費(甲R佔~89%)、也讓乙配吃甲物流~11%；歸正後兩邊各自吃自己的。
function momoMonthFeeSplit(month){
  const ck='__split|'+month;
  ...
  try{
    ...
    if(sum && sum.E!=null && sum.A){
      ...
      const shared=E-甲物流-乙物流-乙倉租;
      if(A>0) split={ E, A, A_inv, 甲R, 乙R, 甲物流, 乙物流, 乙倉租, shared, sharedRate:shared/A,
        feeRate甲: 甲R>0?(甲物流/甲R+shared/A):(E/A),
        feeRate乙: 乙R>0?((乙物流+乙倉租)/乙R+shared/A):(E/A),
        yiZucuRate: 乙R>0?乙倉租/乙R:0 };   // 乙倉租按乙配店內營收攤（per-SKU 待 C1212）
    }
  }catch(e){}
  _momoFeeRateCache[ck]=split;
  return split;
}
...
function momoAggregatePeriods(product,periodKeys,shop){
  ...
  if(momoIsMoPlus(shop)){
    ...
    //   營收＝售價×qty（實測＝A 貨款口徑）；毛利＝營收−估算費用(近3月MO+實際總費率)−COGS(原廠編號查成本)；標 e001Estimated。
    //   ⚠ 混合期別（部分已結算）→ 不覆蓋、走下方結算路徑（不混估算與實際）。對帳明細一到該期別即回結算路徑＝完全取代、不相加。
    if(periodKeys && periodKeys.length){
      const _ctx=momoMoPlusE001Ctx(shop), _ed=_ctx.doc;
      const _allUnsettled=periodKeys.every(k=>!_ctx.settled.has(k));
      if(_allUnsettled && _ed && _ed.hasPrice && _ed.bySkuRev && _ed.bySkuRev[product.sku]){
        let eRev=0, eQty=0; periodKeys.forEach(k=>{ eRev+=(_ed.bySkuRev[product.sku][k]||0); eQty+=((_ed.bySku[product.sku]&&_ed.bySku[product.sku][k])||0); });
        if(eRev>0 || eQty>0){
          const origin=(_ed.skuOrigin&&_ed.skuOrigin[product.sku])||product.origin||'';
          const costMap=momoMoPlusCostMapCached(); const covered=!!(origin && Number(costMap[origin])>0);
          const cogs=covered?Number(costMap[origin])*eQty:0;
          const efr=_ctx.estFee, feeRateEst=efr?efr.rate:null;
  ...
  if(shop!=='甲配' && shop!=='乙配'){
    // ── 未指定 shop 的舊呼叫（非 MO+、非甲乙）：原公式，一字不改 ──
  ...
  //   甲配：C1202 已逐期別，直接取；乙配：C1204 月總 → 按「選期別C1105訂購量 ÷ 整月C1105訂購量」拆(半月按銷量、跟營收拆同軌)。
  let skuFreightPeriod=null;
  if(reconciled){ const frt=momoMonthFreightInfo(shop, month);
    if(frt){
      if(frt.splitByQty && frt.yiMonthly){
        const monthYi=frt.yiMonthly[sku]||0;
        if(monthYi>0){
          const monthKeys=[month+'-H1', month+'-H2'];
          const gMonth=monthKeys.map(k=>(product.periods||{})[k]).filter(Boolean).map(c=>momoReadCell(c,sku)).reduce((s,d)=>s+(d.qty||0),0);
          skuFreightPeriod = gMonth>0 ? monthYi*(qty/gMonth) : ((periodKeys.length>=2)?monthYi:0);   // 無C1105比例可拆：整月給全額、半月不亂拆給0
        } else skuFreightPeriod=0;
      } else {
        skuFreightPeriod=periodKeys.reduce((s,pk)=>s+((frt.freight[sku]&&frt.freight[sku][pk])||0),0);
      }
    }
  }
...
function momoRenderRecon(shop){
  ...
  c.innerHTML=`
    ...
    <div style="margin-bottom:12px">
      ...
      <div class="mm-uprow"><div class="mm-uplbl">供應商對帳單-Excel <span class="mm-code">C1101</span><span class="req">*必要</span><div class="mm-hint">逐 SKU 營收、對帳數量、客退、保留款 · 上傳時程：每月 11 號</div></div><div class="mm-upctl"><input type="file" accept=".xls,.xlsx" onchange="momoReconPick(event,'xls','${shop}')">${stat(_momoReconStage.xlsName)}</div></div>
  ...
  try{
    ...
    if(shop==='甲配'){
      ...
      if(fi && fi.reconciled && fi.shopRev>0 && split && fr && fr.yiSku && Object.keys(fr.yiSku).length){
        // C1204 逐SKU月總(半月於 aggregate 按 C1105 訂購量拆)；乙倉租+shared 併進 otherRate 按乙配店內營收攤
        info={ reconciled:true, logiTotal:split.乙物流, feeRate:fi.feeRate, shopRev:fi.shopRev,
               otherRate:(split.yiZucuRate+split.sharedRate), yiMonthly:fr.yiSku, yiTotal:fr.total, splitByQty:true };
      }
...
function momoFeeExplainerHTML(){
  ...
  return `<div style="margin-top:14px;max-width:460px">
    ...
    <div class="mm-note" style="margin-top:8px;line-height:2">
      物流各歸該店（甲配 C1202／乙配 C1204 逐SKU）、乙倉租歸乙配、其餘按營收攤；月加總 = 對帳單 E。<br>
...
function momoAllPeriods(shop){
  ...
  if(momoIsMoPlus(shop)){   // MO+：對帳明細期別權威在 origins docs（products.periods 本機常空）；再補 E001 未結算期別 → 總表當月看得到
    try{ momoSettledPeriods(shop).forEach(k=>{ if(/^\d{4}-\d{2}-H[12]$/.test(k)) set.add(k); }); }catch(e){}
    try{ const ed=momoLoadE001(shop); if(ed&&ed.byPeriodSold) Object.keys(ed.byPeriodSold).forEach(k=>{ if(/^\d{4}-\d{2}-H[12]$/.test(k) && !momoMoPlusE001Ctx(shop).settled.has(k)) set.add(k); }); }catch(e){}
  }
...
// MOMO 百分比統一：一律小數第一位（0%→0.0%、100%→100.0%，整數不再參差）。非數字回原樣（'—'/'zerodiv' 等照顯示）。比照 momoMoney，全站設計統一會沿用。
function momoPct(v){ return (typeof v==='number'&&isFinite(v)) ? v.toFixed(1)+'%' : (v==null?'—':String(v)); }
...
// 訂單編號 14 碼 → 下單日 'YYYYMMDD'（可排序字串，跨檔比「誰比較新」用）。同 momoMoPlusOrderToPeriod 的 chars[1:6]。非法回 null。
function momoMoPlusOrderDate(orderNo){
  const s=String(orderNo==null?'':orderNo).trim();
  if(!/^6\d{13}$/.test(s)) return null;
  const y=2020+Number(s[1]), mn=Number(s.slice(2,4)), dn=Number(s.slice(4,6));
  if(mn<1||mn>12||dn<1||dn>31) return null;
...
function momoMoPlusResolveCols(header){
  ...
  idx.freeShip=find(['免運活動服務費']);   // D 內單項：經濟上是運費補貼成本，運費行註記用（不從 SKU 毛利抽出、D 整包留 SKU）
...
function momoParseMoPlus(rows, srcCode){
  ...
  // ── B 運費代收 per-SKU 攤分：逐訂單按「同訂單商品營收 A」比例攤到該訂單各商品列（同訂單共享運費、不跨訂單、不用期別總）。存 ln.bAlloc ──
  //   ⚠ 純退運費／無可攤商品營收（該訂單有運費 B 但無商品列，或商品 ΣA≤0）→ 無法攤，記入 freightOnly 回報（不靜默丟棄、不平均攤）。
  const byOrder={};
  lines.forEach(ln=>{ const g=byOrder[ln.orderNo]=byOrder[ln.orderNo]||{prod:[], frB:0}; if(ln.freight){ g.frB+=Number(ln.B)||0; } else { g.prod.push(ln); } });
  const freightOnly={n:0, b:0, orders:[]};   // 有運費 B 但無可攤商品營收（純退運費等）
  let bAllocTot=0;
  Object.keys(byOrder).forEach(on=>{ const g=byOrder[on]; if(!g.frB) return;              // 該訂單無運費收入 → 免攤
    const sumA=g.prod.reduce((s,l)=>s+(Number(l.A)||0),0);
    if(g.prod.length===0 || sumA<=0){ freightOnly.n++; freightOnly.b+=g.frB; freightOnly.orders.push({orderNo:on, b:Math.round(g.frB), prodLines:g.prod.length}); return; }
    g.prod.forEach(l=>{ const share=(Number(l.A)||0)/sumA; l.bAlloc=(l.bAlloc||0)+g.frB*share; bAllocTot+=g.frB*share; });   // 按 A 比例攤（帶號：退貨負 A → 負攤，同訂單守恆）
  });
  freightOnly.b=Math.round(freightOnly.b);
...
/* ═══════════════ E001 未結算銷量（每週更新用；純邏輯）═══════════════
   E001 訂單明細（表頭在第 1 列 / index 0；對帳明細表頭在 index 2 → 餵錯檔在此 fail-loud）。
   ⚠ 架構底線（使用者定案）：只做「銷量」，不碰金額（售價/成交手續費/進帳等財務欄一律不讀不存）；
     絕不寫入 momo_moplus_origins、不進 compact；獨立本機儲存、先不上雲；
     某期別一旦有對帳明細 → 該期別 E001 不顯示、不加總（取代非相加）。
   ⚠ 只取 5 欄：訂單編號 / 訂單狀態 / 商品編號 / 商品原廠編號 / 數量。期別＝訂編 chars[1:6]（複用 momoMoPlusOrderToPeriod）。*/
// 狀態白名單（可維護常數；momo 若新增狀態名，未在名單者計「其他」並附註列出、不靜默丟）：
const MOMO_E001_SOLD_STATUS=['配送結束'];                                    // ✅ 計入未結算銷量（與對帳明細「已完成單」口徑一致）
const MOMO_E001_INTRANSIT_STATUS=['已印單待寄件','商品配達物流中心','待客戶取件'];   // 🚚 在途：不計銷量、顯示附註筆數/數量
const MOMO_E001_EXCLUDED_STATUS=['取消訂單','已回收','退貨中','取消退貨'];        // ✋ 取消/退貨：不計、不附註
const MOMO_E001_COLS={ order:['訂單編號'], status:['訂單狀態'], sku:['商品編號'], origin:['商品原廠編號'], qty:['數量'], price:['商品售價'] };
//   ⚠ price（col40 商品售價）：實測逐 SKU × 數量 = 對帳明細 A 貨款（完全相同口徑、非折扣前售價）→ 當未結算月營收估算，比照甲乙進價×qty。
function momoParseE001(rows){
  if(!Array.isArray(rows)||rows.length<2) return {ok:false, errors:['E001「訂單明細」列數不足']};
  const header=(rows[0]||[]).map(x=>String(x==null?'':x).trim());
  const idx={}, missing=[];
  Object.keys(MOMO_E001_COLS).forEach(k=>{ const i=header.findIndex(h=>MOMO_E001_COLS[k].includes(h)); idx[k]=i; if(k!=='price'&&i<0) missing.push(k+'('+MOMO_E001_COLS[k][0]+')'); });   // price 選填：缺則營收估算不可用、銷量仍可
  if(missing.length) return {ok:false, errors:['E001 找不到必要欄位：'+missing.join('、')+'。（表頭應在第 1 列；若這是「對帳明細」請改用上方對帳明細入口。）'], header:header.filter(Boolean).slice(0,12)};
  const bySku={}, bySkuRev={}, skuOrigin={}, byPeriodSold={}, byPeriodRev={};
  const intransit={n:0,qty:0}, excluded={n:0}, unknown={};
  let soldRows=0, soldQty=0, soldRev=0, minDate=null, maxDate=null;
  const hasPrice=idx.price>=0;
  for(let r=1;r<rows.length;r++){
    const row=rows[r]; if(!row||row.every(c=>c===''||c==null)) continue;
    const order=String(row[idx.order]==null?'':row[idx.order]).trim(); if(!order) continue;
    const od=momoMoPlusOrderDate(order); if(od){ if(!minDate||od<minDate)minDate=od; if(!maxDate||od>maxDate)maxDate=od; }
    const status=String(row[idx.status]==null?'':row[idx.status]).trim();
    if(MOMO_E001_SOLD_STATUS.includes(status)){
      const sku=String(row[idx.sku]==null?'':row[idx.sku]).trim(); const period=momoMoPlusOrderToPeriod(order);
      if(!sku || !period) continue;
      const qty=momoMoPlusNum(row[idx.qty]), origin=String(row[idx.origin]==null?'':row[idx.origin]).trim();
      const rev=hasPrice?momoMoPlusNum(row[idx.price])*qty:0;   // 營收估算＝售價×數量（＝A 貨款、實測驗證）
      (bySku[sku]=bySku[sku]||{})[period]=(bySku[sku][period]||0)+qty;
      (bySkuRev[sku]=bySkuRev[sku]||{})[period]=(bySkuRev[sku][period]||0)+rev;
      if(origin && origin!=='無' && !skuOrigin[sku]) skuOrigin[sku]=origin;
      byPeriodSold[period]=(byPeriodSold[period]||0)+qty; byPeriodRev[period]=(byPeriodRev[period]||0)+rev;
      soldRows++; soldQty+=qty; soldRev+=rev;
    } else if(MOMO_E001_INTRANSIT_STATUS.includes(status)){ intransit.n++; intransit.qty+=momoMoPlusNum(row[idx.qty]); }
    else if(MOMO_E001_EXCLUDED_STATUS.includes(status)){ excluded.n++; }
    else { const key=status||'(空白)'; unknown[key]=(unknown[key]||0)+1; }   // ⚠ 白名單外未知狀態：不靜默丟、計「其他」並附註
  }
  return { ok:true, bySku, bySkuRev, skuOrigin, byPeriodSold, byPeriodRev, hasPrice, soldRows, soldQty, soldRev, intransit, excluded, unknown,
    cutoff:maxDate, srcRange:{from:minDate,to:maxDate}, skuN:Object.keys(bySku).length };
}
// MO+ 未結算月費率估算：近 3 月已對帳「總費率」＝(Dtot+Ctot−Btot)/Atot 平均（成交手續費+其他D+運費淨/A）。
//   ⚠ 用 MO+ 自己的（實測 16.73% 量級），不是甲乙的 6.8%——後者用在 MO+ 會高估毛利率近 10pp（危險方向）。
//   不足 3 月用現有月；fallback＝最近一個完整結算月；標來源月份。回 {rate, months:[], mode} 或 null（無 recon）。
function momoMoPlusEstFeeRate(shop){
  const seen={};
  const add=(m, rec)=>{ if(m in seen) return; const s=rec&&rec.sys; if(s&&s.Atot>0){ seen[m]=(Number(s.Dtot||0)+Number(s.Ctot||0)-Number(s.Btot||0))/s.Atot; } };
  const scan=(store)=>{ if(!store) return; const pf='ec_momo_reconcile|'+shop+'|'; Object.keys(store).forEach(k=>{ if(k.indexOf(pf)===0) add(k.slice(pf.length), store[k]); }); };
  try{ scan(Store._profitMem); scan(Store._mem); }catch(e){}
  try{ for(let i=0;i<localStorage.length;i++){ const k=localStorage.key(i); const pf='ec_momo_reconcile|'+shop+'|'; if(k&&k.indexOf(pf)===0){ try{ add(k.slice(pf.length), JSON.parse(localStorage.getItem(k))); }catch(e){} } } }catch(e){}
  const months=Object.keys(seen).sort().reverse();   // 新→舊
  if(!months.length) return null;
  const use=months.slice(0,3);
  const rate=use.reduce((s,m)=>s+seen[m],0)/use.length;
  return { rate, months:use, mode:(use.length>=3?'近3月':'近'+use.length+'月')+'已對帳實際總費率' };
}
// ── E001 未結算銷量本機儲存（per-shop per-source-month 分片，比照 momo_moplus_origins）：ec_momo_e001|<shop>|<src> ──
//   分片＝重上傳同一份檔案為冪等（整 doc 取代、不累加）、大小有界。走 _markPending → __cloudE001.setSrc 上雲（Keani 同意後新增）。
//   期別歸屬：每個期別 P（月份 M）只由「來源月 src==M」的 doc 提供（一份檔的跨月尾巴由各自月份的 doc 主掌）→ 同期別跨檔不雙計。
function momoE001Key(shop, src){ return 'ec_momo_e001|'+shop+'|'+src; }
function momoLoadE001Doc(shop, src){   // _profitMem → _mem → localStorage（比照 momoLoadMoPlusOriginsDoc）
  const k=momoE001Key(shop, src);
  try{ if(typeof Store!=='undefined'&&Store._profitMem&&Store._profitMem[k]) return Store._profitMem[k]; }catch{}
  try{ if(typeof Store!=='undefined'&&Store._mem&&Store._mem[k]) return Store._mem[k]; }catch{}
  try{ const l=localStorage.getItem(k); if(l) return JSON.parse(l); }catch{}
  return null;
}
function momoSaveE001Doc(shop, src, docObj){
  const k=momoE001Key(shop, src); let lsOk=true;
  try{ localStorage.setItem(k, JSON.stringify(docObj)); }catch(e){ lsOk=false; try{ console.error('%c[E001] 本機儲存失敗（配額？）→ 呼叫端須出可見警告：'+k,'color:#dc2626;font-weight:700', e); }catch{} }
  try{ if(Store._profitMem) Store._profitMem[k]=docObj; }catch{}
  try{ if(Store._mem) Store._mem[k]=docObj; }catch{}
  _markPending(k);   // 待同步 → __cloudE001.setSrc（sweep 白名單也含 ec_momo_e001|）
  try{ momoBumpMoPlusEpoch(); }catch(e){}
  return lsOk;   // false → 不靜默吞配額，呼叫端要出可見警告
}
function momoListE001Docs(shop){   // src(YYMM) → doc（跨三鏡像聯集；只認 4 碼來源月，比照 momoListMoPlusOriginsDocs）
  const pref='ec_momo_e001|'+shop+'|', out={};
  const isMonthSrc=s=>/^\d{4}$/.test(s);
  const scan=st=>{ if(!st)return; Object.keys(st).forEach(kk=>{ if(kk.indexOf(pref)===0){ const src=kk.slice(pref.length); if(isMonthSrc(src) && out[src]==null) out[src]=st[kk]; } }); };
  try{ scan(Store._profitMem); scan(Store._mem); }catch{}
  try{ for(let i=0;i<localStorage.length;i++){ const kk=localStorage.key(i); if(kk&&kk.indexOf(pref)===0){ const src=kk.slice(pref.length); if(isMonthSrc(src) && out[src]==null){ try{ out[src]=JSON.parse(localStorage.getItem(kk)); }catch{} } } } }catch{}
  return out;
}
function momoClearE001(shop){   // 清所有來源月 doc（本機三鏡像；雲端不動、要清雲端另走）
  const pref='ec_momo_e001|'+shop+'|', ks=[];
  try{ for(let i=0;i<localStorage.length;i++){ const kk=localStorage.key(i); if(kk&&kk.indexOf(pref)===0) ks.push(kk); } }catch{}
  try{ if(Store._profitMem) Object.keys(Store._profitMem).forEach(kk=>{ if(kk.indexOf(pref)===0 && ks.indexOf(kk)<0) ks.push(kk); }); }catch{}
...
function momoMergeE001Docs(docs){
  ...
  Object.keys(docs||{}).sort().forEach(src=>{ const d=docs[src]; if(!d) return; out.srcs.push(src); if(d.hasPrice) out.hasPrice=true;
    ...
    const br=d.byPeriodRev||{}; Object.keys(br).forEach(p=>{ if(own(p)) out.byPeriodRev[p]=(out.byPeriodRev[p]||0)+br[p]; });
    const bs=d.bySku||{}; Object.keys(bs).forEach(sku=>{ Object.keys(bs[sku]||{}).forEach(p=>{ if(own(p)){ (out.bySku[sku]=out.bySku[sku]||{})[p]=(out.bySku[sku][p]||0)+bs[sku][p]; } }); });
    const bsr=d.bySkuRev||{}; Object.keys(bsr).forEach(sku=>{ Object.keys(bsr[sku]||{}).forEach(p=>{ if(own(p)){ (out.bySkuRev[sku]=out.bySkuRev[sku]||{})[p]=(out.bySkuRev[sku][p]||0)+bsr[sku][p]; } }); });
    if(d.intransit){ out.intransit.n+=(d.intransit.n||0); out.intransit.qty+=(d.intransit.qty||0); }
    if(d.excluded){ out.excluded.n+=(d.excluded.n||0); }
    if(d.unknown){ Object.keys(d.unknown).forEach(s=>{ out.unknown[s]=(out.unknown[s]||0)+d.unknown[s]; }); }
    if(d.cutoff && (!out.cutoff || d.cutoff>out.cutoff)) out.cutoff=d.cutoff;
  });
  return out;
}
...
//   ⚠ MO+ 的對帳明細資料權威在**來源月 origins docs**（momo_moplus_origins；products.periods 本機常為空、不可靠）→ 主取 origins docs 的期別，
//     再 union products.periods（若某路徑有寫）當保險。任一來源出現該期別即視為已結算。
function momoSettledPeriods(shop){
  const s=new Set();
  try{ const docs=momoListMoPlusOriginsDocs(shop); Object.values(docs).forEach(d=>{ const sk=(d&&d.skus)||{}; Object.keys(sk).forEach(sku=>Object.keys(sk[sku]||{}).forEach(pd=>s.add(pd))); }); }catch(e){}
  try{ (momoLoadProducts(shop)||[]).forEach(p=>{ if(p&&p.periods) Object.keys(p.periods).forEach(pd=>s.add(pd)); }); }catch(e){}
  return s;
}
...
function momoParseMoPlusMaster(rows){
  ...
  for(let r=2;r<rows.length;r++){
    const row=rows[r]; if(!row || row.every(c=>c===''||c==null)) continue;
    const sku=momoCleanSku(row[idx.sku]); if(!sku || !momoSkuValid(sku)) continue;   // 共用 sku 防護：清隱形字 + 白名單（髒 sku 不建）
...
//
//  ⚠ 這兩支目前只有「console 入口」、無 UI（掛在 window，見檔尾 Object.assign）。使用方式：
//    1) 先掃描（唯讀、零風險）：  momoMoPlusScanDirty('MO+麻吉')
//       → 看回傳 summary（五類異常計數）+ 各異常陣列裡的 idx / sku / hasData / visualized（隱形字碼位）。
//    2) 確認要刪哪幾筆後再清理：  momoMoPlusCleanDirty('MO+麻吉', [617, ...])   // 傳 scan 給的 idx
//       → 四道防呆全過才刪：符合筆數需相符、hasData=true（有銷售）整批擋下、master 版擋下、__cloudMomo 未就緒擋下；
//         通過才本機刪 + __cloudMomo.setShop 整包取代雲端（避免雲端 sync 回舊資料）。回 {ok,removed,remaining,cloud,scope}。
//  之所以留在 code（而非用完即棄的 console snippet）：森之旅上線後若再出現同類髒資料，同一套工具可直接重用。
...
// src → doc（跨三鏡像聯集）。⚠ 只回「來源月」doc（src=YYMM 4 碼）；同 collection 的 `master` doc（商品主檔規格層）
//   src='master'、非 4 碼 → **排除**，避免被 index/一致性/完整性/運費等「掃所有來源月」邏輯當成某個月的資料。
function momoListMoPlusOriginsDocs(shop){
...
// 商品主檔規格層 doc（掛牌價/市價/規格名，~370KB 顯示用、不進金額計算）＝存 momo_moplus_origins 的 `master` doc
//   （key ec_momo_moplus_origins|shop|master、cloud docId mo_maji_master），與來源月 doc 並存、同 __cloudMoPlusOrigins.setSrc 同步、零 firebase.js。
//   重傳整份取代（冪等）。讀寫走既有 origins doc 函式（src='master'）。
function momoLoadMoPlusMaster(shop){ return momoLoadMoPlusOriginsDoc(shop, 'master'); }
...
function momoBuildMoPlusOriginsDoc(parsed, shop, allowedPeriods){
  ...
  //   ②只取正常銷售列——排除運費列 + 訂單狀態非「已送達」（回收確認/退貨的負/0 售價會誤導）；只收已建檔 SKU。
  const latestSale={};
  (parsed.lines||[]).forEach(ln=>{
    if(ln.freight) return;                        // 排除運費列
    if(ln.status!=='已送達') return;              // 排除退貨/回收/其他狀態
    if(!has.has(ln.sku)) return;                  // 只收已建檔
    if(!ln.orderDate) return;                     // 無下單日不比
...
function momoMoPlusIndexCached(shop){
  ...
  if(!c || c.epoch!==_moDataEpoch){ c=_moIdxCache[shop]={epoch:_moDataEpoch, built:momoBuildMoPlusOriginsIndex(shop)}; }
  return c.built;
}
...
let _moMasterCache=null, _moMasterEpoch=-1, _moMasterShop=null;
function momoMoPlusMasterCached(shop){   // 商品主檔 doc（掛牌價/市價/規格）每 epoch 讀一次；render 迴圈共用
  if(_moMasterCache && _moMasterEpoch===_moDataEpoch && _moMasterShop===shop) return _moMasterCache;
...
function momoOpenSpecPrices(shop, sku){
  const specs=((momoMoPlusMasterCached(shop)||{})[sku]||{}).specs||[];
  const bestOrigin=momoMoPlusBestOriginForSku(shop, sku);
  const specLabel=s=>[s.spec1,s.spec2].filter(x=>x&&x!=='無').join('/')||'（無規格）';
  const sales=(momoMoPlusIndexCached(shop).originTot||{})[sku]||{};   // 按原廠編號的累計銷量
  let ov=document.getElementById('momo-specprice-overlay');
  if(!ov){ ov=document.createElement('div'); ov.id='momo-specprice-overlay'; document.body.appendChild(ov); }
  ...
  ov.onclick=e=>{ if(e.target===ov) ov.remove(); };
  const rows=specs.length?specs.map(s=>{ const best=(bestOrigin&&s.origin===bestOrigin), q=sales[s.origin]||0;
    return `<tr${best?' style="background:#fff7ed"':''}><td style="padding:5px 10px">${best?'★ ':''}${_momoEsc(specLabel(s))}<span style="color:#c7cad1;font-size:11px"> ${_momoEsc(s.origin||'')}</span></td><td style="padding:5px 10px;text-align:right;font-weight:${best?'700':'400'}">${Number(s.listPrice)>0?momoMoney(s.listPrice):'—'}</td><td style="padding:5px 10px;text-align:right;color:#9ca3af">${Number(s.msrp)>0?momoMoney(s.msrp):'—'}</td><td style="padding:5px 10px;text-align:right;color:#9ca3af">${q||'—'}</td></tr>`; }).join('')
    :`<tr><td colspan="4" style="text-align:center;color:#9ca3af;padding:16px">此商品主檔無規格資料</td></tr>`;
  ov.innerHTML=`<div class="ana-modal" style="width:min(560px,96vw);max-height:85vh;display:flex;flex-direction:column" onclick="event.stopPropagation()">
    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 18px;border-bottom:1px solid #eef0f3">
      <div><div style="font-size:15px;font-weight:800">各規格掛牌價</div><div style="font-size:12px;color:#6b7280;margin-top:2px">${_momoEsc(momoShopDisplay(shop))}｜品號 ${_momoEsc(sku)}｜★＝銷售最多（售價欄預設）</div></div>
      <button class="mm-linkbtn" onclick="document.getElementById('momo-specprice-overlay').remove()">關閉</button>
    </div>
    <div style="overflow:auto;padding:4px 18px 14px">
      <table class="mm-ptbl" style="width:100%;border-collapse:collapse;margin-top:8px">
        <thead><tr style="color:#6b7280;font-size:12px;border-bottom:2px solid #eef0f3"><th style="text-align:left;padding:5px 10px">規格</th><th style="text-align:right;padding:5px 10px">掛牌價</th><th style="text-align:right;padding:5px 10px">市價</th><th style="text-align:right;padding:5px 10px">累計銷量</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <div style="font-size:12px;color:#9ca3af;margin-top:10px;line-height:1.7">掛牌價＝最近一次上傳商品主檔的快照值（無生效日、重傳取代）。售價欄預設顯示銷售最多的規格。</div>
    </div>
  </div>`;
}
function momoMoPlusMarginCalc(inp){   // 純函式：{qty, revA, originsQty:{origin:qty}, feeD, feeC, feeB, costMap:{origin:cost}} → 毛利 + 涵蓋率
  const revA=Number(inp.revA)||0, feeD=Number(inp.feeD)||0, feeC=Number(inp.feeC)||0, feeB=Number(inp.feeB)||0, originsQty=inp.originsQty||{}, costMap=inp.costMap||{};
...
function momoMoPlusFreightForPeriod(shop, periodKeys){
  const docs=momoListMoPlusOriginsDocs(shop); let b=0,c=0,fs=0,rows=0;
  Object.values(docs).forEach(d=>{ const fr=(d&&d.freight)||{}; (periodKeys||[]).forEach(k=>{ const f=fr[k]; if(f){ b+=Number(f.b)||0; c+=Number(f.c)||0; fs+=Number(f.freeShip)||0; rows+=Number(f.rows)||0; } }); });
  return { b:Math.round(b), c:Math.round(c), freeShip:Math.round(fs), net:Math.round(b-c), rows };
}
// ② 完整度（規則式）：期別結束日 +12 天是否已到（momo：客收貨隔天起10天猶豫期滿+撥入信託才計入；實測入帳−出貨≥11天）→ 未到＝結算中。
function momoPeriodSettleDue(periodKey, nowTs){
  const m=/^(\d{4})-(\d{2})-(H1|H2|FULL)$/.exec(periodKey||''); if(!m) return null;
  const y=+m[1], mo=+m[2], half=m[3];
...
function momoMoPlusMargin(shop, sku, period){   // 包裝：讀 cell(qty/revA) + origins(origin/D) + cost_by_origin(live)
  ...
  const {originsQty, feeD, feeC, feeB}=momoMoPlusOriginsForSku(shop, sku, period);
  return momoMoPlusMarginCalc({qty:rc.qty, revA:rc.revUntax, originsQty, feeD, feeC, feeB, costMap:momoLoadCostByOrigin()});
}
...
function momoRenderShop(shop){
  ...
  el.innerHTML=`
    <div style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap;align-items:center">${pills}
      ...
      <button id="momo-exp-btn-${shop}" onclick="momoExportExcel('${shop}')" style="padding:5px 14px;border-radius:7px;font-size:13px;font-weight:600;border:1px solid #e5e7eb;background:#fff;color:#6b7280;cursor:pointer">⬇ 匯出 Excel</button>
    </div>
...
async function momoOpenSyncPreview(shop){
  ...
  // 🔴 MO+ E001 未結算銷量讀 momo_e001 collection（與寫入 __cloudE001.setSrc 同源）——讀寫同源、計數與資料同一份 items。
  const e001Cloud={};   // 'ec_momo_e001|<shop>|<src>' → docData|undefined 或 {__error:true}
  const e001Items=items.filter(it=>it.kind==='MO+未結算銷量');
  if(e001Items.length && window.__cloudE001){
    await Promise.all(e001Items.map(async it=>{
      const pp=it.key.split('|');   // ['ec_momo_e001', shop, src]
      try{ const s=await window.__cloudE001.getDoc(pp[1], pp[2]); e001Cloud[it.key]= s.exists()?(s.data()||{}):undefined; }
...
function momoRenderSyncPreviewModal(shop, items){
  ...
  const statusCell=it=>{
    if(it.status==='new'){
      if(it.suspicious) return `<span style="color:#dc2626;font-weight:700" title="本機 ${it.localCount} 筆、但雲端讀到 0——筆數這麼多卻雲端全空，很可能是雲端讀取失敗、不是真的新增。整包推上去會清空雲端 → 確認前請勿推送（已預設不勾）。">⚠ 雲端讀取可疑，預設不推</span>`;
      return `<span style="color:#10b981;font-weight:600">新增</span>`;
    }
...
// 依賣場可用的欄（MO+ 專屬欄 moPlusOnly 只給 MO+；noMoPlus 欄 MO+ 不給）——「顯示條件分支」，不改欄位結構本身
function momoColAvail(shop){ const mp=momoIsMoPlus(shop); return MOMO_PROFIT_COLS.filter(c=>{ if(c.moPlusOnly&&!mp) return false; if(c.noMoPlus&&mp) return false; return true; }); }
function momoColDefKeys(shop){ return momoColAvail(shop).filter(c=>!c.fixed).map(c=>c.k); }   // 非固定欄的預設順序（依賣場可用）
...
function momoColsSeenSet(){
  ...
  const seed=MOMO_PROFIT_COLS.map(c=>c.k).filter(k=>!_MOMO_NEW_COLS.includes(k));
  try{ localStorage.setItem(_MOMO_COLS_SEEN_LS, JSON.stringify(seed)); }catch{}
  return new Set(seed);
}
function momoColIsNew(k){ return !momoColsSeenSet().has(k); }
function momoColMarkAllSeen(){ try{ localStorage.setItem(_MOMO_COLS_SEEN_LS, JSON.stringify(MOMO_PROFIT_COLS.map(c=>c.k))); }catch{} }
function momoLoadColCfg(shop){
  let cfg={order:momoColDefKeys(shop), hidden:[]};
...
function momoDisplayCols(shop){
  ...
  const avail=new Set(momoColAvail(shop).map(c=>c.k));   // 依賣場過濾（MO+ 專屬/排除欄）
  const hidden=new Set(cfg.hidden);
  const nameCol=MOMO_PROFIT_COLS.find(c=>c.fixed);
  const out=nameCol?[nameCol]:[];
  cfg.order.forEach(k=>{ if(byK.has(k)&&!hidden.has(k)&&avail.has(k)) out.push(byK.get(k)); });
  return out;
}
...
function momoRenderColPicker(shop){
  ...
  m.innerHTML=`<div style="padding:6px 13px 4px;font-size:11px;color:#9ca3af;font-weight:700;display:flex;justify-content:space-between;align-items:center">欄位 <span>${vis}/${cols.length}</span></div>`
    +cols.map(c=>`<div class="cp-row" draggable="true"
      ...
      <span class="cp-row-handle">⠿</span>
      <input type="checkbox" ${!hidden.has(c.k)?'checked':''} style="margin:0;pointer-events:none"> ${c.label}${momoColIsNew(c.k)?' <span style="font-size:10px;font-weight:700;color:#fff;background:#ef4444;border-radius:4px;padding:0 5px;margin-left:4px">新</span>':''}
    </div>`).join('')
    +`<div style="padding:4px 13px 6px;border-top:1px solid #e5e7eb;text-align:right;display:flex;gap:10px;justify-content:flex-end">
...
function momoOpenColPicker(shop,btn){
  ...
  momoRenderColPicker(shop);   // 先渲染（這次仍看得到「新」標記）
  try{ momoColMarkAllSeen(); }catch(e){}   // 開過面板即視為看過 → 下次不再標新、不再跳提示
...
const _MOMO_CMP_GOOD={view:1, convRate:1, qty:1, returnRate:-1};   // 只保留：瀏覽量/成交率/本期銷量/退貨率（營收/毛利率/毛利貢獻 不做環比）
...
let _momoRenderedRows={};   // shop → { period, rows, cols }：最近一次總表渲染結果，供「匯出 Excel」讀（匯出==畫面看到的）
// ⚡ 排序/搜尋/欄位切換的「聚合免重算」快取：完整重繪時存下「已烘焙 agg×2N + 甲乙物流重分配 + 標籤」的基準列（搜尋/篩選/排序之前）。
//   tableOnly 呼叫（排序 8255 / 搜尋 8243 / 欄位切換 8125…）重用此份、只重跑 filter+sort+HTML → 排序不再退化成「≥完整重繪」。
//   period 為 key；所有「資料變更」路徑都走完整重繪（tableOnly falsy）→ 快取自然重建，無 stale（比照 momoTagsFor 既有 tableOnly 快取準則）。
let _momoBaseRows={};   // shop → { period, rows, stock:{at,stale,dateStr,ageDays} }
// 總表匯出 Excel：用最近渲染的那份（已套當前期別/搜尋/篩選/排序），欄位＝畫面顯示欄；另補 品號/原廠編號 兩欄。
function momoExportExcel(shop){
  const snap=_momoRenderedRows[shop];
  if(!snap || !snap.rows || !snap.rows.length){ alert('目前沒有可匯出的資料，請先開啟「商品獲利總表」。'); return; }
  if(typeof XLSX==='undefined' || !XLSX.utils || typeof XLSX.writeFile!=='function'){ alert('匯出元件未載入，請重新整理後再試。'); return; }
  const {rows, cols, period}=snap;
  const dispCols=cols.filter(c=>c.k!=='name');   // name 欄拆成 品號/原廠/商品名 三欄，不重複
  const header=['品號','原廠編號','商品名稱',...dispCols.map(c=>c.label)];
  const aoa=[header];
  rows.forEach(r=>{
...
function momoRenderProfitBody(shop, tableOnly){
  ...
  // E001 未結算估算期別旗標：整期別皆無對帳明細且 E001 有售價資料 → 全列走估算覆蓋。用於「不與結算期別環比/對比」（口徑不同、point 四）。
  const _e001Est = (momoIsMoPlus(shop) && period) ? (()=>{ try{ const c=momoMoPlusE001Ctx(shop), pk=momoExpandPeriod(period); return !!(pk.length && pk.every(k=>!c.settled.has(k)) && c.doc && c.doc.hasPrice && pk.some(k=>c.doc.byPeriodSold&&c.doc.byPeriodSold[k])); }catch(e){ return false; } })() : false;
  ...
  if(tableOnly && _base && _base.period===period){
    ...
    _stockAt=_base.stock.at; _stockStale=_base.stock.stale; _stockDateStr=_base.stock.dateStr; _stockAgeDays=_base.stock.ageDays;
  } else {
    momoClearFeeRateCache();   // 每次完整重繪清月費率快取 → 吃到最新對帳單（快取只在這一輪 1300 SKU 內共用）
    // 「自家庫存」快照（帳號級、按原廠編號；甲乙共用同一批）：載一次供整表用，避免逐 cell 讀 localStorage
    const _stockObj=momoLoadStockByOrigin(); const _stockMap=(_stockObj&&_stockObj.byOrigin)||{}; _stockAt=(_stockObj&&_stockObj.uploadedAt)||0;
    _stockAgeDays=_stockAt?Math.floor((Date.now()-_stockAt)/86400000):null;
    _stockStale=(_stockAgeDays!=null && _stockAgeDays>MOMO_STOCK_STALE_DAYS);
    _stockDateStr=_stockAt?(()=>{const d=new Date(_stockAt);return (d.getMonth()+1)+'/'+d.getDate();})():'';   // 短格式 8/8（去前導0），省欄寬
    // 新欄位一次性提示（每 session 一次，直到使用者開過欄位面板）：讓人知道總表多了欄，位置維持補在最尾、不強制插入
    if(!tableOnly){ try{ const nc=MOMO_PROFIT_COLS.filter(c=>momoColIsNew(c.k)); if(nc.length && !window.__momoNewColToasted){ window.__momoNewColToasted=true; if(typeof showToast==='function') showToast('總表新增欄位：'+nc.map(c=>c.label).join('、')+'（在右上「欄位」可調整位置/顯示）','info'); } }catch(e){} }
  ...
  // 共用篩選（標籤 OR + 數值 AND）疊加在搜尋/上下架之後。基數=篩選前可見數、N=篩選後 → 總覽標「已篩選 N / 基數」。
  const _filterOn=momoFilterActiveCount(shop)>0;
  ...
  _momoRenderedRows[shop]={ period, rows, cols };   // 存這次渲染的列（已搜尋/篩選/排序）→「匯出 Excel」用同一份，保證匯出==畫面
  const fmt={ money:v=>momoMoney(v), num:v=>Math.round(v).toLocaleString(), pct1:v=>momoPct(v) };
  ...
  const buildRowHtml=r=>{
    const tds=cols.map(c=>{
      ...
      if(c.k==='tags'){
        ...
        return `<td class="tl" style="overflow:hidden">${momoTagChipHTML(r._tags, _periodIsHalf)}</td>`;
      }
      if(c.k==='returnRate'){
      ...
      if(c.k==='convRate'){
        ...
        return `<td style="${st}" title="對帳數量 ${Math.round(r.qty||0)} ÷ 瀏覽 ${Math.round(r.view||0)}${r.viewEstimated?'（半月沿用整月·估算）':''}">${momoPct(cr)}${momoRowCmp('convRate',r,r._prev)}</td>`;
      }
      if(c.k==='margin'){
        // 營收0但有毛利（整批退貨）→ margin=null → 「—」，不顯示誤導的 0%；負毛利標紅（是負值，不是 0）
        // ⚠ 未結算估算列（e001Estimated）比照甲配未對帳月：數字照常顯示（一般樣式、無 ~/估/斜體）；估算僅由頂端橫幅標示。
      ...
      if(c.k==='stock'){
        // 自家庫存（帳號級快照·按原廠編號·甲乙共用）：無 origin/查無→「—」；有值→數字+tooltip（共用註記+資料日期），過期橘字
        if(!_stockAt) return `<td style="text-align:right;color:#c7cad1" title="尚未上傳莫筆克庫存資料（各倉商品列表）">—</td>`;
  ...
  // ⚡ per-row HTML 快取：排序/搜尋不改 row 內容（只換順序/子集）→ 重用；欄位集變動（colSig 變）或資料變更（完整重繪重置 _momoBaseRows）才重建。
  //   colSig 只看欄位鍵＋順序：td 不含寬度（寬度在 colgroup），故拖曳欄寬不必重建 row HTML。真正熱點（8000 格字串生成）於排序時降為「查表＋重排 join」。
  const colSig=cols.map(c=>c.k).join(',');
  const _bcache=_momoBaseRows[shop];
  let rowHtmlBySku;
  if(_bcache && _bcache.period===period && _bcache.rowHtmlBySku && _bcache.colSig===colSig){
    rowHtmlBySku=_bcache.rowHtmlBySku;   // 同 period + 同欄位集 → 重用（排序/搜尋）
  } else {
    rowHtmlBySku={};
    const baseRows=(_bcache && _bcache.rows) || rows;   // 用「基準全集」建（搜尋子集也查得到）；完整重繪時 _bcache.rows 剛存好
    baseRows.forEach(r=>{ rowHtmlBySku[r.sku]=buildRowHtml(r); });
    if(_bcache){ _bcache.rowHtmlBySku=rowHtmlBySku; _bcache.colSig=colSig; }
  }
  const tbody=rows.map(r=>{ const h=rowHtmlBySku[r.sku]; return h!==undefined?h:buildRowHtml(r); }).join('');   // 顯示子集依序查表；理論上都命中，未命中則現算（安全網）
  ...
  if(!tableOnly){
    ...
    if(isJiaYi && period){
      if(fi&&fi.reconciled){
        ...
        if(shop==='甲配'){
          ...
          if(_freightMode==='precise') statusChip+=` <span class="mm-status ok" title="寄倉物流按 C1204 逐SKU精算歸戶；乙物流已全額歸乙配(不再由甲配補貼)、倉租+其他按乙配店內營收攤">🟢 物流精算</span>`;
          else statusChip+=` <span class="mm-status no" title="此月無 C1204 寄倉運費逐SKU資料，乙物流仍按乙配店內營收比例攤（估算）→ 到訂單明細上傳 C1204 升級">🟡 無 C1204·營收攤估算</span>`;
        }
      } else {
        const h=momoHistoricalFeeRate(shop);
    ...
    const curT= _filterOn ? momoTotalsFromRows(rows) : momoPeriodTotals(shop, period);
    let prevKey= _e001Est ? '' : momoPrevPeriodKey(period);   // 未結算估算期別：不與結算期別對比（口徑不同、point 四）→ KPI「vs 上期」顯示「—」
    ...
    if(momoIsMoPlus(shop) && rows.length){
      let dismissed=false; try{ dismissed=localStorage.getItem('ec_momo_price_hint')==='1'; }catch(e){}
      const emptyN=rows.filter(r=>r.latestSale==null).length;
      if(!dismissed && emptyN >= rows.length*0.5){
        moPlusPriceBanner=`<div class="mm-banner mm-banner-warn" style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;margin-bottom:8px">
          <span>💲 <b>售價資料需重新上傳對帳明細後才會出現</b>（${emptyN}/${rows.length} 個商品尚無售價）——「最新售價」是本次新增欄位、既有已上傳資料沒有它，重傳一次 2606/2607（或最新月份）對帳明細即會出現。</span>
          <button class="mm-linkbtn" style="flex-shrink:0" onclick="momoDismissMoPlusPriceHint('${shop}')">知道了</button>
        </div>`;
      }
    }
...
// 商品異動時間軸（product.history；成本/進價/售價 + 名稱/原廠/品號 changes；最後一筆=目前生效）
function momoHistoryTimelineHTML(p){
  ...
  return `<div style="max-height:180px;overflow:auto"><table style="width:100%;border-collapse:collapse;font-size:12px">
    ...
    <tbody>${rows.map(x=>`<tr style="border-top:1px solid #f1f5f9${x._i===curIdx?';background:#ecfdf5':''}">
      ...
      <td style="padding:4px 8px;color:#64748b;text-align:left">${(x.changes&&x.changes.length)?`<span style="color:#7c3aed">${_momoEsc(x.changes.map(c=>c.field+' '+(c.from||'—')+'→'+(c.to||'—')).join('；'))}</span>${x.note?' · ':''}`:''}${_momoEsc(x.note||'')}</td></tr>`).join('')}</tbody></table></div>`;
}
...
// ══════════ MOMO 商品標籤分類（甲乙配各自獨立排名；只在整月期別算；隨期別重算）══════════
//   蝦皮的標籤在同事檔案、不碰；這是 MOMO 自己一套（相對排名，不用絕對金額）。emoji+短字，面板/逐列共用。
const MOMO_TAG_GROUPS=[
  {group:'流量', tags:[
    {k:'traffic_hilo', emoji:'🔵', label:'高流量低轉換', short:'高流低轉', desc:'瀏覽量前 20% 且成交率後 30%'},
    {k:'traffic_lohi', emoji:'🟣', label:'高轉換低流量', short:'高轉低流', desc:'成交率前 20% 且瀏覽量後 30%'},
    {k:'dead_new',     emoji:'🆕', label:'首次零動銷',  short:'首次滯銷', desc:'上架、本期零銷量、上一期有賣 → 需立刻反應（半月＝前一個半月）'},
    {k:'dead_long',    emoji:'⚫', label:'連續零動銷',  short:'連續滯銷', desc:'上架、本期+上一期都零銷量 → 滯銷（半月＝連續兩個半月）'},
  ]},
  {group:'成長', tags:[
    {k:'surge', emoji:'🟢', label:'爆發品', short:'爆發', desc:'營收環比 > +30%（上期需 ≥5 件才計，濾掉小基期假訊號）。僅整月計算（半月環比雜訊大）'},
    {k:'drop',  emoji:'🔴', label:'重跌品', short:'重跌', desc:'營收環比 < −30%（上期需 ≥5 件才計）。僅整月計算（半月環比雜訊大）'},
  ]},
  {group:'營收', tags:[
    {k:'rev_top', emoji:'👑', label:'高營收', short:'高營收', desc:'毛利貢獻前 10%（分母=該期有銷售）'},
    {k:'rev_mid', emoji:'🟠', label:'中營收', short:'中營收', desc:'毛利貢獻前 10~30%'},
...
function momoTagChipHTML(tags, half){
  if(!tags||!tags.length) return '';
  const isTraf=k=>(k==='traffic_hilo'||k==='traffic_lohi');
  const pk=momoPrimaryTag(tags), pt=MOMO_TAG_BY_KEY[pk]; if(!pt) return '';
  const more=tags.length-1;
  const allTxt=tags.map(k=>{const t=MOMO_TAG_BY_KEY[k];return t?t.emoji+' '+t.label+(half&&isTraf(k)?'（估）':''):'';}).filter(Boolean).join('｜')
    +(half?'　·　半月：成長類不計、流量類為估算':'');
  const est=(half&&isTraf(pk))?`<span class="mm-rtag-est" title="半月流量為 S1103 月值拆分估算">估</span>`:'';
  return `<span class="mm-rtag-chip" title="全部標籤：${allTxt}">${pt.emoji}<span class="mm-rtag-t">${pt.short}</span>${est}${more>0?`<span class="mm-rtag-more">+${more}</span>`:''}</span>`;
}
const MOMO_TAG_BY_KEY={}; MOMO_TAG_GROUPS.forEach(g=>g.tags.forEach(t=>{ MOMO_TAG_BY_KEY[t.k]=Object.assign({group:g.group},t); }));
const MOMO_GROWTH_MIN_QTY=5;   // 成長類最低基期訂購量：上期 <5 件不算成長率（見 momoComputeTags 成長段說明）
// 「緊鄰的前一期別」：半月→前一個半月(H2→同月H1、H1→上月H2)、整月→上月整月。用於零動銷「連續」判定，配合每半月上傳一次的優化節奏。
//   （與 momoPrevPeriodKey 不同：那個是環比用的「半月比上月同半月」；這裡要的是時間序上真正的前一期。整月時兩者相同。）
function momoPrevSeqPeriodKey(periodKey){
  const m=/^(\d{4})-(\d{2})-(H1|H2|FULL)$/.exec(periodKey||''); if(!m) return '';
  let y=+m[1], mo=+m[2], half=m[3];
  const prevMonth=()=>{ mo--; if(mo<1){ mo=12; y--; } };
  if(half==='H2') return `${y}-${String(mo).padStart(2,'0')}-H1`;
  if(half==='H1'){ prevMonth(); return `${y}-${String(mo).padStart(2,'0')}-H2`; }
  prevMonth(); return `${y}-${String(mo).padStart(2,'0')}-FULL`;   // FULL → 上月整月（= momoPrevPeriodKey）
}
// 計算某賣場某期別的逐商品標籤。整月＋半月都算，只有「成長類(爆發/重跌)」僅整月（半月環比一個大單就 ±50%、雜訊大）。
//   半月是實際優化節奏(每半月傳一次 C1105)，故流量/營收/利潤/零動銷都要能在半月看。
//   「連續零動銷」的連續＝緊鄰的前一期別(半月＝連續兩個半月、整月＝連續兩個整月，見 momoPrevSeqPeriodKey)；上期有賣本期停＝首次(需立刻反應)。
//   ⚠ 半月的流量來自 S1103 月值拆分＝估算(顯示層標「估」)；退貨率來自對帳單月值，半月取不到就不標(不當 0)。
function momoComputeTags(shop, periodKey){
  const isFullMonth = !!(periodKey && String(periodKey).endsWith('-FULL'));
  const isHalf = /-(H1|H2)$/.test(String(periodKey||''));
  const isMoPlus = momoIsMoPlus(shop);   // MO+ 缺成本＝成本涵蓋<100%（非 product.cost），與總表口徑一致
  const bySku={}, counts={}; MOMO_TAG_GROUPS.forEach(g=>g.tags.forEach(t=>counts[t.k]=0));
  if(!(isFullMonth||isHalf)) return {isFullMonth:false, isHalf:false, bySku, counts};
  const keys=momoExpandPeriod(periodKey);
  const priorKey=momoPrevSeqPeriodKey(periodKey);   // 緊鄰前一期(半月→前一半月)，配合每半月上傳的優化節奏
  const priorOk = !!(priorKey && priorKey.slice(0,7)>=MOMO_FIRST_PERIOD);   // 上線首月前(2025/12)資料不完整→不當上期
  const priorKeys = priorOk ? momoExpandPeriod(priorKey) : null;
  // ⚠ 只算「上架」商品：總表預設也只顯示上架，下架品進排名會扭曲百分位、且讓「上架＝有銷+零動銷」對不上。
  //   下架品一律不給標籤（隱藏中，標了看不到；顯示已下架時無標籤＝已退場，合理）。
  const all=momoLoadProducts(shop).filter(p=>p.discontinued!==true);
  const num=x=>(typeof x==='number' && isFinite(x));
  const R=all.map(p=>{
    const a=momoAggregatePeriods(p, keys, shop);
    const prev=priorKeys?momoAggregatePeriods(p, priorKeys, shop):null;
    return {sku:p.sku, p, a, prev, active:p.discontinued!==true, sold:(a.grossQty>0||Math.abs(a.revenue)>0.5)};
  });
  const add=(sku,k)=>{ if(!sku) return; (bySku[sku]=bySku[sku]||[]).push(k); counts[k]++; };
  // 流量類：母體=有 S1103 瀏覽量且成交率可算的商品。相對排名 pctile。
  const traf=R.filter(r=>num(r.a.view)&&num(r.a.convRate));
  if(traf.length>=5){
    const vs=traf.map(r=>r.a.view).sort((x,y)=>x-y), cs=traf.map(r=>r.a.convRate).sort((x,y)=>x-y);
    const pv=f=>vs[Math.min(vs.length-1,Math.floor(f*vs.length))], pc=f=>cs[Math.min(cs.length-1,Math.floor(f*cs.length))];
    const vHi=pv(0.8), vLo=pv(0.3), cHi=pc(0.8), cLo=pc(0.3);
    traf.forEach(r=>{ if(r.a.view>=vHi&&r.a.convRate<=cLo) add(r.sku,'traffic_hilo'); if(r.a.convRate>=cHi&&r.a.view<=vLo) add(r.sku,'traffic_lohi'); });
  }
  // 零動銷（上架、本期零銷量）→ 首次 vs 連續
  R.forEach(r=>{ if(!r.active||r.sold) return;
    if(priorOk && r.prev && (r.prev.grossQty>0||Math.abs(r.prev.revenue)>0.5)) add(r.sku,'dead_new'); else add(r.sku,'dead_long'); });
  // 成長類（需上期有營收，否則不標——避免把新品標成爆發/把停售標成重跌）
  //   ⚠ 僅整月計算：半月環比雜訊大（一個大單就 ±50%）→ 半月不標爆發/重跌。
  //   ⚠ 最低基期門檻 MOMO_GROWTH_MIN_QTY：上期實際訂購量 < 5 件不算成長率。
  //     理由：小基期讓百分比失真（賣 1→0＝−100%、3→5＝+67% 全是雜訊）——六月甲配未設門檻時 surge67/drop90 有 78% 來自上期<5件的品，
  //     設 ≥5 後降到 surge15/drop20（35/234≈15%），才是真的「量放大/量萎縮」而非小數字跳動。用訂購量(grossQty)不用淨量，避免退貨把基期壓掉。
  if(isFullMonth && priorOk){ R.forEach(r=>{ if(!r.prev) return; const pr=r.prev.revenue; if(!(pr>0.5)) return;
    const base=(r.prev.grossQty!=null?r.prev.grossQty:r.prev.qty);
    if(!(base>=MOMO_GROWTH_MIN_QTY)) return;
    const g=(r.a.revenue-pr)/Math.abs(pr); if(g>0.3) add(r.sku,'surge'); else if(g<-0.3) add(r.sku,'drop'); }); }
  // 營收類：毛利貢獻(profit)排名，分母=該期有銷售
  const soldR=R.filter(r=>r.sold).slice().sort((x,y)=>(y.a.profit||0)-(x.a.profit||0));
  const n=soldR.length;
  soldR.forEach((r,i)=>{ const f=n?i/n:1; if(f<0.1) add(r.sku,'rev_top'); else if(f<0.3) add(r.sku,'rev_mid'); else if(f<0.5) add(r.sku,'rev_dev'); });
  // 利潤類（只看有銷售）
  const retVals=R.filter(r=>r.sold&&num(r.a.returnRate)).map(r=>r.a.returnRate);
  const avgRet=retVals.length?retVals.reduce((a,b)=>a+b,0)/retVals.length:null;
  R.forEach(r=>{ if(!r.sold) return;
...
// ── 共用篩選狀態（標籤 + 數值 + 搜尋 都疊加、共用一套；階層圖也吃這套）──
const _momoTagFilter={};   // shop → Set(tagKey)（勾選的標籤，OR）
...
function momoTagsFor(shop, recompute){
  ...
  if(!recompute && c && c.period===period) return c.result;
  const result=momoComputeTags(shop, period);
...
function momoRowPassesFilter(shop, r, rowTags){
  ...
  if(nf && nf.length){
    const noBiz = Math.abs(r.revenue||0)<0.5 && !((r.grossQty!=null?r.grossQty:r.qty)>0);   // 本期零營收且零銷量＝無業績
    for(const c of nf){
      // ⚠ 毛利率是比率：無業績時 aggregate 給的 margin=0 是「無法計算」不是真 0%（同零營收「—」的坑）→ 排除，不撈進「<20」。
...
// ── 標籤 / 篩選 面板（沿用 col-picker-menu 外殼；MOMO 自有 JS）──
function momoOpenFilterPanel(shop,btn){
  ...
  setTimeout(()=>document.addEventListener('click',function h(e){
    ...
    if(!cur){ document.removeEventListener('click',h); return; }
    if(!cur.contains(e.target)){ cur.remove(); document.removeEventListener('click',h); }
  }),0);   // 點面板外關閉（延後掛，避免這次點擊立刻觸發）
}
function momoCloseFilterPanel(shop){ const m=document.getElementById('momo-filter-'+shop); if(m) m.remove(); }
function momoTagToggle(shop,key){
  const s=(_momoTagFilter[shop]=_momoTagFilter[shop]||new Set());
  if(s.has(key)) s.delete(key); else s.add(key);
  momoRenderProfitBody(shop,false); momoRenderFilterPanel(shop);
}
function momoNumAdd(shop){
  const col=document.getElementById('momo-nf-col-'+shop), op=document.getElementById('momo-nf-op-'+shop);
  const ai=document.getElementById('momo-nf-a-'+shop), bi=document.getElementById('momo-nf-b-'+shop);
  if(!col||!op) return;
  const a=parseFloat(ai&&ai.value);
  if(!isFinite(a)){ if(ai){ ai.style.borderColor='#dc2626'; ai.focus(); } return; }
  const cond={k:col.value, op:op.value, a};
  if(op.value==='between'){
    const b=parseFloat(bi&&bi.value);
    if(!isFinite(b)){ if(bi){ bi.style.borderColor='#dc2626'; bi.focus(); } return; }
    cond.a=Math.min(a,b); cond.b=Math.max(a,b);
  }
  (_momoNumFilter[shop]=_momoNumFilter[shop]||[]).push(cond);
  momoRenderProfitBody(shop,false); momoRenderFilterPanel(shop);
  const nai=document.getElementById('momo-nf-a-'+shop); if(nai) nai.focus();   // 加入後聚焦回值格，方便連續加多條
}
// 輸入框有值但未加入時，把「加入」鈕點亮成主色 + 顯示「↵ 套用」提示 → 讓人看得出條件尚未生效（不重繪、避免掉焦點）。
function momoNumPendingSync(shop){
  const ai=document.getElementById('momo-nf-a-'+shop), bi=document.getElementById('momo-nf-b-'+shop);
  const btn=document.getElementById('momo-nf-addbtn-'+shop), hint=document.getElementById('momo-nf-pending-'+shop);
  const has=!!((ai&&ai.value.trim())||(bi&&bi.value.trim()));
  if(ai) ai.style.borderColor='';   // 清掉之前驗證失敗的紅框
  if(btn) btn.classList.toggle('pending', has);
  if(hint) hint.style.display=has?'inline':'none';
}
function momoNumRemove(shop,idx){
  const nf=_momoNumFilter[shop]; if(!nf) return; nf.splice(idx,1);
  momoRenderProfitBody(shop,false); momoRenderFilterPanel(shop);
}
...
function momoRenderFilterPanel(shop){
  ...
  const counts=tagsRes.counts||{};
  const tf=_momoTagFilter[shop]||new Set();
  const full=!!tagsRes.isFullMonth;
  const total=momoLoadProducts(shop).length;
  ...
  const tagHTML=MOMO_TAG_GROUPS.map(g=>{
    ...
    const tagsHtml = g.tags.map(t=>{
      ...
      const c=counts[t.k]||0; const on=tf.has(t.k); const dis=(c===0&&!on);
      return `<button class="mm-fp-tag${on?' on':''}${dis?' dis':''}"${dis?' disabled':''} title="${t.emoji} ${t.label}｜${t.desc}" onclick="momoTagToggle('${shop}','${t.k}')">${t.emoji} ${t.label} <span class="mm-fp-cnt">${c}</span></button>`;
    }).join('');
  ...
  const condHTML=nf.length? nf.map((c,i)=>{
    const col=colByK[c.k]||{label:c.k,unit:''};
  ...
  const addRow=`<div class="mm-fp-add">
    <select id="momo-nf-col-${shop}" class="mm-sel mm-fp-sel">${colOpts}</select>
    <select id="momo-nf-op-${shop}" class="mm-sel mm-fp-sel" style="width:64px" onchange="momoNumPendingSync('${shop}')"><option value="gt">&gt;</option><option value="lt">&lt;</option><option value="between">區間</option></select>
    <input id="momo-nf-a-${shop}" class="mm-fp-inp" type="number" step="any" placeholder="值" ${kd}>
    <input id="momo-nf-b-${shop}" class="mm-fp-inp" type="number" step="any" placeholder="上限" ${kd}>
    <button id="momo-nf-addbtn-${shop}" class="mm-fp-addbtn" onclick="momoNumAdd('${shop}')">加入</button>
  ...
  m.innerHTML=`
    ...
    <div class="mm-fp-body">
      <div class="mm-fp-sec-h">標籤（甲乙各自排名／可多選 OR${full?'':'；半月：成長類停用、流量類估算'}）</div>
      ${tagHTML}
    ...
    <div class="mm-fp-foot">
      ...
      <button class="mm-fp-done" onclick="momoCloseFilterPanel('${shop}')">完成</button>
    </div>`;
}
// 工具列 🏷 標籤/篩選 按鈕的作用中狀態（殼不重繪，篩選變動後手動同步）
function momoSyncFilterChip(shop){
  const b=document.getElementById('momo-filter-chip-'+shop); if(!b) return;
  const n=momoFilterActiveCount(shop);
  b.className='mm-chip'+(n?' on':'');
  b.style.cssText=n?'background:#5b5fcf;border-color:#5b5fcf;color:#fff':'';
  b.innerHTML='🏷 標籤 / 篩選'+(n?'（'+n+'）':'');
}
function momoHierarchyStats(shop){
  ...
  // 疊加共用篩選（標籤 + 數值）：階層圖母體跟總表一致
  const tfOn=momoFilterActiveCount(shop)>0;
  const tags=tfOn?momoTagsFor(shop):null;
  const bins={lt0:0,b0_10:0,b10_15:0,b15_20:0,b20_30:0,gt30:0};
  let withMargin=0, zeroRev=0, passCount=0;
  filtered.forEach(p=>{
    const a=momoAggregatePeriods(p, keys, shop);
    if(tfOn){
      const rowTags=(tags.bySku&&tags.bySku[p.sku])||[];
      const rowObj={...a, ppUntax:(Number(p.purchasePrice)||0)/1.05, salePrice:p.salePrice||0, cost:Number(p.cost)||0, discontinued:!!p.discontinued};
      if(!momoRowPassesFilter(shop, rowObj, rowTags)) return;
    }
    passCount++;
  ...
  return {period, filtered:(tfOn?passCount:filtered.length), total:all.length, withMargin, zeroRev, bins, ge20, lt20};
}
...
function momoOpenHierarchy(shop){
  ...
  const pct=n=> den>0?momoPct(n/den*100):'—';
...
function momoRenderBatchEdit(shop){
  ...
  const _anCtx=momoBatchAnomalyCtx(shop);   // MO+：預算 costMap+masterMap 供整迴圈共用；甲乙=null
  all.forEach(p=>{ const a=momoProductAnomalies(p,shop,_anCtx); if(a.cost){ cnt.nocost++; if(period&&(shop==='甲配'||shop==='乙配')&&momoHasPeriodRevenue(p,shop,period)) cnt.nocostP++; } if(a.pp)cnt.nopp++; if(a.sp)cnt.nosp++; if(p.discontinued===true)cnt.disc++; });
  ...
  body.innerHTML=`
    <div class="mm-row" style="margin-bottom:8px">
      ...
      ${allBtn}${nocostChips}${momoIsMoPlus(shop)?'':fbtn('缺進價','nopp',cnt.nopp,'#d97706')}${fbtn('缺售價','nosp',cnt.nosp,'#d97706')}
    ...
    ${(momoIsMoPlus(shop)&&(filt==='nocost'||filt==='nocostP'))?momoRenderMissingCostPanel(shop):''}
    <div class="mm-row" style="margin-bottom:10px">
...
function momoRenderBatchEditList(shop){
  ...
  const _anCtx=momoBatchAnomalyCtx(shop);   // MO+：預算 costMap+masterMap；甲乙=null
  let list=all;
  ...
  if(filt) list=list.filter(p=>{ const a=momoProductAnomalies(p,shop,_anCtx);
  ...
  el.innerHTML=list.slice(0,50).map(p=>{
    const on=p.sku===sel, an=momoProductAnomalies(p,shop,_anCtx);
    let badges='';
...
// 品號能不能改：只有「完全無歷史/關聯」才安全（改主鍵會讓以 sku 索引的 對帳單/運費/S1103/optlog 斷鏈）。
//   保守：任一命中就當有歷史 → 唯讀。回 {has, reasons:[...]}（reasons 供 UI 說明為什麼不能改）。
function momoSkuHistoryInfo(shop, sku){
  const reasons=[]; if(!sku) return {has:false, reasons};
  const p=momoLoadProducts(shop).find(x=>x.sku===sku);
  const moSet=new Set(); if(p&&p.periods) Object.keys(p.periods).forEach(k=>{ if(/^\d{4}-\d{2}-H[12]$/.test(k)) moSet.add(k.slice(0,7)); });
  if(moSet.size) reasons.push(moSet.size+' 個月銷售紀錄');
  const recSet=new Set(), rf='ec_momo_reconcile|'+shop+'|';
  const scanRec=st=>{ if(!st) return; try{ Object.keys(st).forEach(k=>{ if(k.indexOf(rf)===0){ const rec=st[k]; if(rec&&rec.skus&&rec.skus[sku]) recSet.add(k.slice(rf.length)); } }); }catch(e){} };
  ...
  try{ for(let i=0;i<localStorage.length&&!hasFrt;i++){ const k=localStorage.key(i); if(k&&k.indexOf(ff)===0){ try{ const fr=JSON.parse(localStorage.getItem(k)); if(fr&&((fr.freight&&fr.freight[sku])||(fr.yiSku&&fr.yiSku[sku]))) hasFrt=true; }catch(e){} } } }catch(e){}
  if(hasFrt) reasons.push('運費資料');
  let hasS=false, sf='ec_momo_s1103|';
  const scanS=st=>{ if(!st) return; try{ Object.keys(st).forEach(k=>{ if(k.indexOf(sf)===0){ const s=st[k]; if(s&&s.skus&&s.skus[sku]) hasS=true; } }); }catch(e){} };
  try{ scanS(Store._profitMem); scanS(Store._mem); }catch(e){}
  if(hasS) reasons.push('排行榜資料');
...
function momoRenderBatchEditForm(shop){
  ...
  if(momoIsMoPlus(shop)) return momoRenderMoPlusBatchEditForm(shop, p);   // MO+：無進價、成本靠原廠自動帶、售價來自商品主檔（甲乙走下方原路徑不變）
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
      ...
      <div><label style="${_MOMO_LB}">原廠編號</label><input id="momo-edit-origin-${shop}" type="text" value="${_momoEsc(p.origin||'')}" style="${_MOMO_INP}"><div style="font-size:11px;color:#9ca3af;margin-top:3px">改原廠編號會重新關聯甲乙配跨通路</div></div>
    </div>
...
function momoBatchSubmitEdit(shop){
  if(momoIsMoPlus(shop)) return momoMoPlusBatchSubmitEdit(shop);   // MO+ 走專屬送出（名稱/原廠/品號+原因，不驗成本/進價/售價；甲乙路徑不變）
  const products=momoLoadProducts(shop);
  ...
  const nameEl=document.getElementById('momo-edit-name-'+shop), originEl=document.getElementById('momo-edit-origin-'+shop), skuEl=document.getElementById('momo-edit-sku-'+shop);
  const newName=nameEl?nameEl.value.trim():(p.name||'');
  const newOrigin=originEl?originEl.value.trim():(p.origin||'');
  const newSku=skuEl?skuEl.value.trim():p.sku;   // 無 input＝品號唯讀＝不變
  if(!(cost>=0)||!(pp>=0)||!(sp>=0)){ alert('成本 / 進價 / 售價需為 ≥0 的數字'); return; }
  if(!newName){ alert('商品名稱不可空白'); return; }
  if(!note){ alert('請填異動原因'); return; }   // 名稱/編號的修改同樣必填異動原因（同一送出鈕套用全部）
  const skuChanged=newSku!==p.sku;
  if(skuChanged){
    if(!newSku){ alert('品號不可空白'); return; }
  ...
  const originChanged=changes.some(c=>c.field==='原廠編號');
  p.name=newName; p.origin=newOrigin; if(skuChanged){ p.sku=newSku; _momoBatchSel[shop]=newSku; }
  p.cost=cost; p.purchasePrice=pp; p.salePrice=sp;
  p.history=p.history||[];
  const entry={...momoNowParts(),cost,purchasePrice:pp,salePrice:sp,note};
  ...
  let costHint='';
  if(originChanged && newOrigin){ try{ const map=momoLoadCostByOrigin(); const mc=map&&map[newOrigin]; if(mc!=null && Number(mc)!==Number(cost)) costHint='新原廠編號「'+newOrigin+'」在莫筆克成本表為 $'+mc+'（目前成本 $'+cost+'）。成本未自動更動 —— 要更新請在成本欄自行改並記錄異動原因。'; }catch(e){} }
  momoRenderBatchEditList(shop); momoRenderBatchEditForm(shop);   // 名稱/品號改動 → 左側清單也要刷新
  if(costHint){ if(window.App&&typeof App.showAlertModal==='function') App.showAlertModal({title:'原廠編號已更新',message:'跨通路關聯已依新原廠重新計算。\n\n⚠ '+costHint,kind:'info'}); else alert('⚠ '+costHint); }
  if(typeof showToast==='function') showToast('已更新並記錄一筆歷程','success');
}

// ── MO+（代收代付）專屬編輯表單：無進價；成本靠原廠編號自莫筆克成本表帶入(唯讀)、售價取自商品主檔掛牌價(唯讀)；可改 名稱/原廠/品號 ──
function momoRenderMoPlusBatchEditForm(shop, p){
  const el=document.getElementById('momo-batch-form-'+shop);
  if(!el) return;
  const costMap=momoMoPlusCostMapCached();
  const origs=(p.origins&&p.origins.length)?p.origins:(p.origin?[p.origin]:[]);
  const costLines=origs.length
    ? origs.map(o=>{ const c=costMap[o]; const oesc=_momoEsc(o);
        if(Number(c)>0) return `<div style="font-size:12px;color:#374151">原廠 ${oesc}：成本 $${c}</div>`;
        const oid=String(o).replace(/[^A-Za-z0-9]/g,'_');   // 查無成本 → 就地補（不切別頁）
        return `<div style="font-size:12px;color:#dc2626">原廠 ${oesc}：查無成本 <a onclick="momoCostInlineToggle('${oesc}')" style="color:#5b5fcf;cursor:pointer;font-weight:600">＋補成本</a>
          <div id="mm-cost-inline-${oid}" style="display:none;margin-top:4px;padding:6px 8px;background:#fff7ed;border:1px solid #fed7aa;border-radius:6px">
            <input id="mm-cost-inline-c-${oid}" type="number" placeholder="成本" style="width:70px;padding:2px 6px;border:1px solid #e5e7eb;border-radius:5px;font-size:12px">
            <input id="mm-cost-inline-n-${oid}" type="text" placeholder="異動原因（必填）" style="width:160px;padding:2px 6px;border:1px solid #e5e7eb;border-radius:5px;font-size:12px" onkeydown="if(event.key==='Enter')momoMoPlusCostInlineSave('${shop}','${_momoEsc(p.sku)}','${oesc}')">
            <button onclick="momoMoPlusCostInlineSave('${shop}','${_momoEsc(p.sku)}','${oesc}')" style="padding:2px 12px;border:none;border-radius:5px;background:#10b981;color:#fff;font-size:12px;font-weight:600;cursor:pointer">存</button>
            <div style="font-size:10px;color:#9a3412;margin-top:3px">⚠ 甲乙+MO+ 共用此成本表：補一次會連動所有用到此原廠編號的 MO+ 商品毛利、各記一筆歷程。已上雲、記得同步。</div>
          </div></div>`;
      }).join('')
  ...
  const hist=(p.history&&p.history.length)? [...p.history].reverse().map(h=>{ const ch=_chTxt(h); return `
    <div style="font-size:12px;padding:6px 10px;border-left:2px solid #e5e7eb;margin-bottom:4px">
      <span style="color:#9ca3af">${h.date||''}${h.time?' '+h.time:''}</span>${ch?`<span style="color:#7c3aed"> · ${_momoEsc(ch)}</span>`:''}${h.note?`<span style="color:#6b7280"> — ${_momoEsc(h.note)}</span>`:''}
    </div>`; }).join('') : `<div style="font-size:12px;color:#9ca3af">尚無歷程</div>`;
  const skuInfo=momoSkuHistoryInfo(shop, p.sku);
  const skuField=skuInfo.has
    ? `<label style="${_MOMO_LB}">品號（主鍵）</label><div style="${_MOMO_INP};background:#f8fafc;color:#64748b;display:flex;align-items:center">${_momoEsc(p.sku||'—')}</div><div style="font-size:11px;color:#9a3412;margin-top:3px">已有 ${_momoEsc(skuInfo.reasons.join('、'))}，品號不可變更（改主鍵會讓對帳單/運費/優化紀錄斷鏈）。需更換請刪除後重建。</div>`
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
      <div><label style="${_MOMO_LB}">商品名稱</label><input id="momo-edit-name-${shop}" type="text" value="${_momoEsc(p.name||'')}" style="${_MOMO_INP}"></div>
      <div><label style="${_MOMO_LB}">原廠編號（主）</label><input id="momo-edit-origin-${shop}" type="text" value="${_momoEsc(p.origin||'')}" style="${_MOMO_INP}"><div style="font-size:11px;color:#9ca3af;margin-top:3px">改原廠編號會改變帶入的成本</div></div>
    </div>
    <div style="margin-bottom:10px">${skuField}</div>
...
// 共用 sku 防護（原批次貼上的正規化，抽出給單筆新增 + 主檔匯入用）：清隱形字/空白，再驗 ^[A-Za-z0-9-]+$。
//   當初 bug：空格分隔把整行吃進 sku → 「TP00011830001244 恆溫暖腳墊…」46 字元髒資料。RegExp 建構子避免 literal 隱形字。
function momoCleanSku(raw){ return String(raw==null?'':raw).replace(new RegExp('[\\s\\u00a0\\u200b-\\u200f\\u202a-\\u202e\\u2060\\u3000\\ufeff]','g'),''); }
function momoSkuValid(sku){ return /^[A-Za-z0-9\-]+$/.test(sku); }
...
function momoMoPlusAddOne(shop){
  ...
  let sku=momoCleanSku(g('sku')?g('sku').value:'');                               // 清隱形字/空白（共用防護）
  const origin=(g('origin')?g('origin').value:'').trim();
  if(!name){ alert('商品名稱必填'); return; }
  const products=momoLoadProducts(shop);
...
const _momoUpFiles={c1105:null,jia:[],yi:null,s1105:null,s1103:null};   // jia 可多檔；s1103=銷售排行榜（帳號級）；yi=C1204 乙配寄倉運費
...
// 甲配主檔各期別已存銷量（估算分攤用）：periods → {period:{sku:qty}}（吃 momoAggregatePeriods，含 compact/sourced 解碼）
function momoJiaStoredQty(periods){
  const out={}; periods.forEach(p=>out[p]={});
  momoLoadProducts('甲配').forEach(p=>{
    periods.forEach(period=>{ const a=momoAggregatePeriods(p,[period],'甲配'); const q=(a&&a.qty)||0; if(q>0) out[period][p.sku]=q; });
  });
  return out;
}
// 甲配運費分攤（混合）：訂編有 C1105 對照 → 逐SKU按數量比例精算；沒有（只傳 C1202）→ 按該期別主檔已存銷量估算。
//   月總永遠 = C1202 該月出貨運費（精算/估算都不變總額；SKU 間分佈精細度才是差異）。
//   回 {freight:{sku:{period:$}}, months:{'YYYY-MM':{mode:'precise'|'est',total,skuN,unmatchedN,noQty}}, badPeriod, unmatchedOrders}
function momoAllocateJiaFreightHybrid(orderFreight, c1105){
  const orderSkuQty=(c1105&&c1105.orderSkuQty)||{};
  const c1105Months=new Set();
  Object.keys(orderSkuQty).forEach(o=>{ const per=momoOrderToPeriod(o); if(per) c1105Months.add(per.slice(0,7)); });
  const freight={}, badPeriod=[], unmatchedOrders=[], estPool={}, mAcc={};
  const touch=m=>(mAcc[m]=mAcc[m]||{total:0,unmatchedN:0,noQty:0});
  Object.keys(orderFreight).forEach(order=>{
    const fee=orderFreight[order]; if(!(fee>0)) return;
    const period=momoOrderToPeriod(order);
    if(!period){ if(badPeriod.length<50) badPeriod.push({order}); return; }
    const mo=period.slice(0,7); touch(mo).total+=fee;
    const skuQty=orderSkuQty[order];
    if(skuQty){
      const tot=Object.values(skuQty).reduce((s,q)=>s+q,0);
      if(tot>0){ Object.keys(skuQty).forEach(sku=>{ (freight[sku]=freight[sku]||{})[period]=(freight[sku][period]||0)+fee*(skuQty[sku]/tot); }); return; }
    }
    // 無 C1105 對照 → 進估算池（按期別）
    estPool[period]=(estPool[period]||0)+fee;
    if(c1105Months.has(mo)){ if(unmatchedOrders.length<50) unmatchedOrders.push(order); mAcc[mo].unmatchedN++; }
  });
  const estPeriods=Object.keys(estPool);
  if(estPeriods.length){
    const qtyMap=momoJiaStoredQty(estPeriods);
    estPeriods.forEach(period=>{
      const pool=estPool[period], skus=qtyMap[period]||{}, tot=Object.values(skus).reduce((s,q)=>s+q,0);
      if(tot>0){ Object.keys(skus).forEach(sku=>{ (freight[sku]=freight[sku]||{})[period]=(freight[sku][period]||0)+pool*(skus[sku]/tot); }); }
      else touch(period.slice(0,7)).noQty+=pool;   // 該期別主檔無銷量 → 無法估算分攤（金額浮出、不亂塞）
    });
  }
  const skuByMonth={}; Object.keys(freight).forEach(sku=>Object.keys(freight[sku]).forEach(per=>{ const m=per.slice(0,7); (skuByMonth[m]=skuByMonth[m]||new Set()).add(sku); }));
  const months={};
  Object.keys(mAcc).forEach(mo=>{ const b=mAcc[mo]; months[mo]={ mode:c1105Months.has(mo)?'precise':'est', total:Math.round(b.total), unmatchedN:b.unmatchedN, noQty:Math.round(b.noQty), skuN:(skuByMonth[mo]?skuByMonth[mo].size:0) }; });
  return {freight, months, badPeriod, unmatchedOrders};
}
// 乙配 UnsendList 單分頁：依品號加總「扣款費用」（不是扣款金額/比例）→ {sku: 該分頁小計}
// 單一「寄倉分攤…運費」分頁 → {品號: 該分頁運費}。容忍 12 種材積變體（欄名不同：扣款費用/合計金額(A+B)/合計/運費）、
//   空變體分頁(只有表頭或總計列)→ 回 {}，不 throw（讓多分頁彙總能繼續）。跳過「總計」列。品號可跨列(不同級距)→ 累加。
function momoParseYiFreightSheet(rows){
  if(!rows || rows.length<2) return {};
  const hdr=(rows[0]||[]).map(x=>String(x).trim());
  const cSku=hdr.indexOf('品號');
  let cFee=-1; ['扣款費用','合計金額(A+B)','合計','運費'].some(h=>{ const i=hdr.indexOf(h); if(i>=0){cFee=i;return true;} return false; });
  if(cSku<0 || cFee<0) return {};   // 這個材積變體分頁沒有可解析欄（多半是空的）→ 略過、不擋整份
  const num=v=>parseFloat(String(v).replace(/,/g,''))||0;
  const out={};
  for(let i=1;i<rows.length;i++){ const r=rows[i]; if(!r) continue; const sku=String(r[cSku]||'').trim(); if(!sku||sku==='總計') continue; out[sku]=(out[sku]||0)+num(r[cFee]); }
  return out;
}
...
function momoBuildUploadPlan(parsed){
  ...
  // 階段四：甲配運費 freight[sku][period]（已由 momoAllocateJiaFreightHybrid 拆好）→ apply 時存獨立 key（不寫 cell、不受 compact 防呆影響）
...
// Dry-run：完全鏡射 momoApplyUploadPlan 的每格邏輯，但「只算不寫」→ 逐賣場逐期別 前/後 qty+rev、新期別 vs 累加、來源標記筆數、變小標紅。
//   給使用者「先對數字再執行、執行後只比對一致」。純讀 master、不動 Store / 不 momoSaveProducts。
function momoUploadDryRun(plan){
  ...
  Object.keys(plan.shops||{}).forEach(shop=>{   // 通用：甲乙配 / MO+ 共用（dry-run 鏡射 writer）
    ...
    Object.keys(sp.updates).forEach(sku=>{
      const p=bySku.get(sku); if(!p) return;
      Object.keys(sp.updates[sku]).forEach(period=>{
        if(!/^\d{4}-\d{2}-H[12]$/.test(period)) return;
        const upd=sp.updates[sku][period]||{};
        const periodCode=momoMonthToCode(period.slice(0,7))||period.slice(2,4)+period.slice(5,7);
        const src=fileCode||periodCode;
        const cell=p.periods&&p.periods[period];
        const rcB=momoReadCell(cell); const bQ=rcB.qty||0, bR=rcB.revUntax||0;   // before（相容 flat/compact）
        const wasCompact=!!(cell&&cell.s!=null);
        const isNew=!(cell&&(cell.s!=null||cell.qty!=null));
        let sources;   // 模擬 after（跟 writer 同一套）
        if(wasCompact){ sources=momoDecodeSources(cell.s); }
        else if(cell&&cell.qty!=null&&Number(cell.qty)>0){ sources={}; sources[periodCode]={qty:Number(cell.qty)||0,rev:0}; }
        else { sources={}; }
        const srcBefore=Object.keys(sources).length;
        sources[src]={qty:Number(upd.qty)||0, rev:Number(upd.rev)||0};
        let aQ=0,aR=0; Object.keys(sources).forEach(k=>{aQ+=sources[k].qty;aR+=sources[k].rev;});
        const e=byPeriod[period]||(byPeriod[period]={period, beforeQty:0,beforeRev:0,afterQty:0,afterRev:0,skuN:0,srcBefore:0,srcAfter:0,anyNew:false,anyAccum:false,shrinkSkus:[]});
        e.beforeQty+=bQ; e.beforeRev+=bR; e.afterQty+=aQ; e.afterRev+=aR; e.skuN++;
        e.srcBefore=Math.max(e.srcBefore,srcBefore); e.srcAfter=Math.max(e.srcAfter,Object.keys(sources).length);
        if(isNew) e.anyNew=true; if(wasCompact) e.anyAccum=true;
        if(aQ<bQ) e.shrinkSkus.push({sku, name:(p.name||'').slice(0,14), beforeQty:bQ, afterQty:aQ});
      });
    });
    out.shops[shop]=Object.keys(byPeriod).sort().reverse().map(k=>{ const e=byPeriod[k];
      e.kind = e.anyNew&&!e.anyAccum ? 'new' : (e.anyAccum ? 'accumulate' : 'flat-migrate');
      e.shrink = (e.afterQty < e.beforeQty) || e.shrinkSkus.length>0;   // 期別合計變小、或任一 SKU 變小（同來源重傳且值更小＝更正/誤判，藏在成長期別裡也要抓出來）
      e.beforeRev=Math.round(e.beforeRev); e.afterRev=Math.round(e.afterRev);
      return e;
    });
  });
  return out;
...
function momoUploadShowDryRun(shop, planOverride){
  ...
  const shrinkAll=[];
  const shopTbl=s=>{ const rows=dr.shops[s]; if(!rows||!rows.length) return '';
    const body=rows.map(r=>{ const dq=r.afterQty-r.beforeQty, drv=r.afterRev-r.beforeRev; if(r.shrink) shrinkAll.push({shop:s,...r});
      return `<tr style="border-top:1px solid #f3f4f6${r.shrink?';background:#fef2f2':''}">
        <td style="padding:4px 8px;white-space:nowrap;font-weight:600">${esc(momoPeriodLabel(r.period))}</td>
        <td style="padding:4px 8px">${kindLabel[r.kind]||r.kind}</td>
        <td style="padding:4px 8px;text-align:right">${r.beforeQty} → <b style="color:${r.shrink?'#dc2626':'#374151'}">${r.afterQty}</b> <span style="color:${dq>0?'#16a34a':(dq<0?'#dc2626':'#9ca3af')}">(${dq>0?'+':''}${dq})</span></td>
        <td style="padding:4px 8px;text-align:right">${r.beforeRev.toLocaleString()} → ${r.afterRev.toLocaleString()} <span style="color:${drv>0?'#16a34a':(drv<0?'#dc2626':'#9ca3af')}">(${drv>0?'+':''}${drv.toLocaleString()})</span></td>
  ...
  ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px';
  ov.onclick=e=>{ if(e.target===ov) ov.remove(); };
  ov.innerHTML=`<div style="background:#fff;border-radius:12px;max-width:880px;width:100%;max-height:88vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.25)">
    ...
    <div style="padding:14px 20px;border-top:1px solid #eef0f2;display:flex;justify-content:space-between;align-items:center">
      ...
      <button onclick="document.getElementById('momo-dryrun-overlay').remove()" style="padding:7px 16px;border-radius:7px;border:1px solid #e5e7eb;background:#fff;color:#6b7280;font-size:13px;cursor:pointer">關閉</button>
    </div>
  </div>`;
}
function momoUploadDryRunCSV(){
  const dr=window.__momoLastDryRun; if(!dr){ alert('請先產生 dry-run'); return; }
  const esc=v=>{ const s=String(v==null?'':v); return /[",\n\r]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s; };
  const lines=['賣場,期別,類型,qty前,qty後,qtyΔ,rev前,rev後,revΔ,來源前,來源後,變小'];
  ['甲配','乙配'].forEach(s=>{ (dr.shops[s]||[]).forEach(r=>{ lines.push([s,r.period,r.kind,r.beforeQty,r.afterQty,r.afterQty-r.beforeQty,r.beforeRev,r.afterRev,r.afterRev-r.beforeRev,r.srcBefore,r.srcAfter,r.shrink?'是':''].map(esc).join(',')); }); });
  try{ const blob=new Blob(['﻿'+lines.join('\r\n')],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='MOMO_dryrun_'+(dr.srcCode||'x')+'_'+new Date().toISOString().slice(0,10)+'.csv'; a.click(); }catch(e){ alert('下載失敗：'+(e&&e.message||e)); }
}
...
function momoRenderMoPlusUpload(shop){
  ...
  const batch=(_moPlusBatch&&_moPlusBatch.shop===shop)?_moPlusBatch:null;   // 批次模式（多選）
  const upStat = batch ? `<span class="mm-ok">✓ 已選 ${batch.items.length} 檔</span><a onclick="momoMoPlusBatchClear('${shop}')" title="清除清單" style="color:#ef4444;cursor:pointer;font-weight:700">✕</a>`
    : (f?`<span class="mm-ok">✓ ${_momoEsc(f.name)}</span><a onclick="momoMoPlusUploadRemove('${shop}')" title="移除" style="color:#ef4444;cursor:pointer;font-weight:700">✕</a>`:`<span class="mm-muted">未選</span>`);
  c.innerHTML=`
    <div style="max-width:820px">
      ...
      <div class="mm-uprow">
        <div class="mm-uplbl">E001 訂單 <span class="mm-code">未結算銷量</span><div class="mm-hint">表頭在第 1 列；勿選對帳明細</div></div>
        <div class="mm-upctl"><input type="file" accept=".xlsx,.xls" onchange="momoE001File('${shop}',event)"><span class="${_e001File?'mm-ok':'mm-muted'}">${_e001File?'✓ '+_momoEsc(_e001File.name):'未選'}</span>${_e001File?`<a onclick="momoE001Remove('${shop}')" title="移除" style="color:#ef4444;cursor:pointer;font-weight:700">✕</a>`:''}</div>
      </div>
      <button class="mm-btn-primary" style="margin-top:10px" onclick="momoE001Generate('${shop}')" ${_e001File?'':'disabled'}>▶ 解析 E001 預覽</button>
      <span class="mm-gen-hint">${_e001File?'解析、顯示未結算銷量與在途/未知狀態（確認再存；存本機並待同步上雲）':'請選 E001 檔'}</span>
      <div id="e001-preview-${shop}" style="margin-top:14px"></div>
      <div id="e001-block-${shop}" style="margin-top:16px">${momoE001BlockHTML(shop)}</div>
    </div>`;
}
let _e001File=null, _e001Parsed=null;
function momoE001File(shop,e){ const files=e.target.files; if(!files||!files.length) return; _e001File=files[0]; _e001Parsed=null; momoRenderMoPlusUpload(shop); }
function momoE001Remove(shop){ _e001File=null; _e001Parsed=null; momoRenderMoPlusUpload(shop); }
async function momoE001Generate(shop){
  if(!_e001File) return;
  const box=document.getElementById('e001-preview-'+shop); if(box) box.innerHTML='<span class="mm-muted">解析中…</span>';
  const srcCode=momoRebuildSrcFromName(_e001File.name);   // 檔名末 4 碼 YYMM → 來源月分片鍵（決定 doc / 期別歸屬）
  if(!srcCode){ if(box) box.innerHTML='<div style="color:#dc2626;font-size:13px">檔名判不出月份（需含 YYMM，例：<b>E001_mo+好麻吉訂單_2608.xls</b>）→ 無法決定來源月分片。</div>'; return; }
  try{
    const wb=await momoReadWorkbook(_e001File);
    const rows=wb.sheet('訂單明細')||wb.firstSheet();
    const parsed=momoParseE001(rows);
    if(!parsed.ok){ if(box) box.innerHTML=`<div style="color:#dc2626;font-size:13px">解析失敗：${_momoEsc((parsed.errors||[]).join('；'))}${parsed.header?`<div class="mm-muted" style="margin-top:4px">實際表頭前幾欄：${_momoEsc(parsed.header.join(' | '))}</div>`:''}</div>`; return; }
    parsed.srcCode=srcCode;
    _e001Parsed=parsed;
    const settled=momoSettledPeriods(shop);
    const shownP=Object.keys(parsed.byPeriodSold).filter(p=>!settled.has(p)).sort();
    const replacedP=Object.keys(parsed.byPeriodSold).filter(p=>settled.has(p)).sort();
    const cutoff=parsed.cutoff&&/^\d{8}$/.test(parsed.cutoff)?(parsed.cutoff.slice(0,4)+'/'+parsed.cutoff.slice(4,6)+'/'+parsed.cutoff.slice(6,8)):'—';
    if(box) box.innerHTML=`<div style="background:#f9fafb;border:1px solid #eef0f4;border-radius:8px;padding:10px 12px;font-size:13px;line-height:1.8">
      <b>解析結果</b>（來源月 <b>${_momoEsc(srcCode)}</b>，截至 ${cutoff}）：配送結束 <b>${parsed.soldRows}</b> 列 / 數量 <b>${parsed.soldQty}</b>、SKU <b>${parsed.skuN}</b> 個。<br>
      期別分布：${Object.keys(parsed.byPeriodSold).sort().map(p=>momoPeriodLabel(p)+' '+parsed.byPeriodSold[p]).join('、')||'—'}<br>
      ${replacedP.length?`<span style="color:#d97706">已有對帳明細（將被取代、不列入）：${replacedP.map(momoPeriodLabel).join('、')}</span><br>`:''}
      ${shownP.length?`<span style="color:#059669">未結算（將顯示）：${shownP.map(momoPeriodLabel).join('、')}</span>`:'<span class="mm-muted">目前所有期別都已有對帳明細 → 存了也不會顯示（全被取代）</span>'}<br>
      在途 ${parsed.intransit.n} 筆、取消/退貨 ${parsed.excluded.n} 筆${Object.keys(parsed.unknown).length?`、<span style="color:#dc2626">未知狀態：${Object.entries(parsed.unknown).map(([s,n])=>_momoEsc(s)+' '+n).join('、')}</span>`:''}
      <div style="margin-top:8px"><button class="mm-btn-primary" onclick="momoE001Apply('${shop}')">✓ 存入（未結算銷量·待同步）</button> <span class="mm-muted" style="font-size:11px">存本機並待同步上雲；同來源月重傳整份取代（冪等）</span></div>
    </div>`;
  }catch(e){ if(box) box.innerHTML='<div style="color:#dc2626">讀取失敗：'+_momoEsc(String(e&&e.message||e))+'</div>'; }
}
function momoE001Apply(shop){
  if(!_e001Parsed||!_e001Parsed.ok) return;
  const p=_e001Parsed;
  const src=p.srcCode;
...
// 未結算銷量顯示區塊：只列「無對帳明細」的期別（有對帳明細＝已結算、取代不顯示）；標未含平台費用/未扣退貨；在途/未知附註。
function momoE001BlockHTML(shop){
  const doc=momoLoadE001(shop);
  if(!doc||!doc.bySku||!Object.keys(doc.bySku).length) return `<div class="mm-muted" style="font-size:12px;border-top:1px solid #f1f5f9;padding-top:12px">尚未有 E001 未結算銷量資料。</div>`;
  const settled=momoSettledPeriods(shop);
  const allP=[...new Set(Object.keys(doc.byPeriodSold||{}))].sort();
  const shownP=allP.filter(p=>!settled.has(p)), replacedP=allP.filter(p=>settled.has(p));
  const cutoff=doc.cutoff&&/^\d{8}$/.test(doc.cutoff)?(doc.cutoff.slice(4,6)+'/'+doc.cutoff.slice(6,8)):'—';
  const rows=[];
  Object.keys(doc.bySku).forEach(sku=>{ let q=0; shownP.forEach(p=>{ q+=doc.bySku[sku][p]||0; }); if(q!==0) rows.push({sku, qty:q, origin:(doc.skuOrigin&&doc.skuOrigin[sku])||''}); });
  rows.sort((a,b)=>b.qty-a.qty);
  const notes=[];
  if(doc.intransit&&doc.intransit.n) notes.push(`🚚 另有在途 <b>${doc.intransit.n}</b> 筆（數量 ${doc.intransit.qty}，尚未配送結束、<b>不計入銷量</b>）`);
  if(doc.unknown&&Object.keys(doc.unknown).length) notes.push(`⚠ 白名單外未知狀態（未計入）：${Object.entries(doc.unknown).map(([s,n])=>_momoEsc(s)+' '+n+' 筆').join('、')}`);
  if(replacedP.length) notes.push(`已有對帳明細的期別 <b>${replacedP.map(momoPeriodLabel).join('、')}</b> 已由結算取代、不列入未結算`);
  const head=`<div style="border-top:1px solid #f1f5f9;padding-top:12px">
    <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap">
      <span style="font-weight:800;font-size:14px">未結算銷量（E001，截至 ${cutoff}）</span>
      <span style="font-size:11px;color:#92400e;background:#fffbeb;border:1px solid #fde68a;border-radius:4px;padding:1px 6px">未含平台費用</span>
      <span style="font-size:11px;color:#92400e;background:#fffbeb;border:1px solid #fde68a;border-radius:4px;padding:1px 6px">未扣除退貨</span>
      <a onclick="momoE001Clear('${shop}')" style="margin-left:auto;font-size:11px;color:#9ca3af;cursor:pointer">清除</a>
    </div>`;
  if(!shownP.length) return head+`<div class="mm-muted" style="font-size:12px;margin-top:8px">所有期別皆已有對帳明細 → 全數由結算取代，無未結算銷量可顯示。</div>${notes.length?`<div style="font-size:11px;color:#6b7280;margin-top:8px;line-height:1.8">${notes.map(n=>'· '+n).join('<br>')}</div>`:''}</div>`;
  const body=rows.slice(0,300).map(r=>`<tr><td style="padding:3px 8px;font-family:monospace">${_momoEsc(r.sku)}</td><td style="padding:3px 8px;color:#9ca3af">${_momoEsc(r.origin)}</td><td style="padding:3px 8px;text-align:right;font-weight:600">${r.qty}</td></tr>`).join('');
  return head+`<div style="font-size:11px;color:#6b7280;margin:4px 0 8px">顯示期別：${shownP.map(momoPeriodLabel).join('、')}｜共 ${rows.length} 個 SKU、數量合計 ${rows.reduce((s,r)=>s+r.qty,0)}</div>
    <div style="max-height:340px;overflow:auto;border:1px solid #eee;border-radius:6px"><table style="width:100%;border-collapse:collapse;font-size:12px">
      <thead><tr style="text-align:left;color:#6b7280;position:sticky;top:0;background:#fafafa"><th style="padding:4px 8px">商品編號</th><th style="padding:4px 8px">原廠編號</th><th style="padding:4px 8px;text-align:right">未結算銷量</th></tr></thead>
      <tbody>${body}</tbody></table></div>
    ${notes.length?`<div style="font-size:11px;color:#6b7280;margin-top:8px;line-height:1.8">${notes.map(n=>'· '+n).join('<br>')}</div>`:''}</div>`;
}
...
function momoRenderMoPlusProductSync(shop){
  ...
  c.innerHTML=`
    <div style="max-width:820px">
      ...
      <div style="font-size:11px;color:#9ca3af;margin-top:16px;line-height:1.7;border-top:1px solid #f1f5f9;padding-top:10px">⚠ 成本靠<b>原廠編號</b>查共用成本表（甲乙+MO+ 共用；甲配每週匯入莫筆克成本檔）。莫筆克沒有的零星原廠編號，到<b>批次維護→編輯商品</b>該原廠「查無成本」列<b>就地補</b>。此頁只上傳商品主檔。</div></div>
    </div>`;
}
...
function momoMoPlusUploadFile(shop,e){ const files=e.target.files; if(!files||!files.length) return;
  if(files.length>1){ momoMoPlusBatchInit(shop, files); return; }   // 多選 → 批次佇列
  _moPlusBatch=null; _moPlusUpFile=files[0]; _moPlusUpParsed=null; _moPlusUpPlan=null; momoRenderMoPlusUpload(shop);   // 單檔 → 原富預覽流程（行為不變）
}
...
// 逐檔「讀→解析→守衛→建 plan」的可重用單元（單檔預覽與批次共用；不碰全域、不寫入）。
//   回 {ok:true, srcCode, parsed, plan}（parsed.ok 且已建 plan）
//   或 {ok:false, srcCode, parsed?, reason, errTitle?, errBody?}——errTitle/errBody 供單檔富預覽；reason 供批次列狀態。
async function momoMoPlusPrepareFile(file, shop){
  const srcCode=momoRebuildSrcFromName(file.name);   // guard ①：檔名 YYMM
  if(!srcCode) return {ok:false, srcCode:null, reason:'檔名判不出月份（YYMM）', errTitle:'檔名判不出月份（YYMM）', errBody:'檔名需含 4 碼月份，例：mo+好麻吉對帳明細_<b>2606</b>.xls。<br>判不出來源鍵會讓「同檔重傳冪等」失效、跨月累加錯亂——請改檔名後重傳，<b>不會預設成當月</b>（猜錯造成錯誤來源鍵）。'};
  let wb; try{ wb=await momoReadWorkbook(file); }catch(err){ const m=String(err&&err.message||err); return {ok:false, srcCode, reason:'讀檔失敗：'+m, errTitle:'讀檔失敗', errBody:_momoEsc(m)}; }
  if(!wb.names || wb.names.indexOf('訂單明細')<0){   // guard ②a：分頁
    return {ok:false, srcCode, reason:'找不到「訂單明細」分頁（可能選成 E001）', errTitle:'這不是對帳明細', errBody:'找不到「訂單明細」分頁（此檔分頁：'+_momoEsc((wb.names||[]).join('、')||'無')+'）。<br>對帳明細與 E001 副檔名相同——你可能傳成 E001 或別的檔。請上傳「mo+…對帳明細_YYMM.xls」。'};
  }
  ...
  if(parsed.errors && parsed.errors.length && /找不到必需欄位/.test(parsed.errors[0])){   // guard ②b：header 格式（擋 E001）
    return {ok:false, srcCode, parsed, reason:'header 格式不符（可能選成 E001）', errTitle:'這不是對帳明細（或格式不符）', errBody:_momoEsc(parsed.errors[0])+'<br><br>對帳明細與 E001 副檔名相同——請確認上傳的是對帳明細（第 3 列 header 應含 訂單號碼／實際入帳金額／貨款…）。'};
  }
  if(!parsed.ok){   // 逐列驗證 / 運費判準不過 → 不建 plan、不可寫（fail loud）
    const reason = (parsed.lineErrors&&parsed.lineErrors.length) ? (parsed.lineErrors.length+' 列 A+B−C−D≠實際入帳（不寫入）')
      : (parsed.freightMismatch&&parsed.freightMismatch.length) ? (parsed.freightMismatch.length+' 列運費判準衝突（不寫入）')
      : '解析未通過（不寫入）';
    return {ok:false, srcCode, parsed, reason};
  }
...
// ══════════════ MO+ 對帳明細「批次上傳」（多選、逐檔依序、任一失敗不中斷、進度顯示）══════════════
//   共用單檔的 momoMoPlusPrepareFile（讀→解析→守衛→建 plan）＋ momoMoPlusApplyPrepared（寫入）＝安全檢查全部保留。
//   全期別寫入（allowedPeriods=null＝比照單檔預設全勾）；MO+ 期別歸屬 danger 恆 false（跨月尾巴累加不覆蓋）。
let _moPlusBatch=null;   // { shop, items:[{name,file,srcCode,status,detail}], running, done, blocked, blockMsg }
function momoYield(){ return new Promise(res=>setTimeout(res,0)); }   // 讓事件迴圈喘一口＋畫面重繪（用 setTimeout 而非 rAF：分頁切到背景時 rAF 會被凍結，批次會卡死）
function momoMoPlusPlanHasShrink(plan){ try{ const g=plan&&plan._guard&&plan._guard.merged; if(!g) return false; return (g.periods||[]).some(p=>(p.skus||[]).some(s=>s.srcShrink)); }catch(e){ return false; } }   // 同來源重傳且量更小＝變小（不阻擋，僅提示）
function momoMoPlusBatchInit(shop, files){
  _moPlusUpFile=null; _moPlusUpParsed=null; _moPlusUpPlan=null;   // 進批次模式：關掉單檔狀態（避免兩套並存）
  const items=[...files].map(f=>({ name:f.name, file:f, srcCode:momoRebuildSrcFromName(f.name), status:'待處理', detail:'' }));
  // ⚠ 重複偵測（擋下、不靜默覆蓋）：檔名重複、或同一期別(srcCode)被選兩次 → 整批不處理、提示改選
  const byName={}, bySrc={};
  items.forEach(it=>{ byName[it.name]=(byName[it.name]||0)+1; if(it.srcCode) bySrc[it.srcCode]=(bySrc[it.srcCode]||0)+1; });
  const dupNames=Object.keys(byName).filter(n=>byName[n]>1);
  const dupSrcs=Object.keys(bySrc).filter(s=>bySrc[s]>1);
  let blocked=false, blockMsg='';
  if(dupNames.length){ blocked=true; blockMsg='檔名重複：'+dupNames.join('、')+'（同一檔被選了兩次）'; }
  else if(dupSrcs.length){ blocked=true; blockMsg='同一期別被選了兩次：'+dupSrcs.map(s=>momoMoPlusSrcToMonth(s)||s).join('、')+'（會互相覆蓋，請只留一個檔）'; }
  items.forEach(it=>{ if(dupNames.includes(it.name)||(it.srcCode&&dupSrcs.includes(it.srcCode))) it.status='重複'; });
  _moPlusBatch={ shop, items, running:false, done:false, blocked, blockMsg };
  momoRenderMoPlusUpload(shop);
}
function momoMoPlusBatchClear(shop){ if(_moPlusBatch&&_moPlusBatch.running) return; _moPlusBatch=null; momoRenderMoPlusUpload(shop); }
function momoMoPlusBatchUpdate(shop){ const prev=document.getElementById('moplus-up-preview-'+shop); if(prev) prev.innerHTML=momoMoPlusBatchQueueHTML(shop); }   // 只重繪佇列容器（不重建 file input）
async function momoMoPlusBatchRun(shop){
...
function momoMoPlusBatchQueueHTML(shop){
  ...
  const esc=_momoEsc;
  const statColor={ '待處理':'#9ca3af','處理中':'#2563eb','成功':'#059669','失敗':'#dc2626','略過':'#9ca3af','重複':'#dc2626' };
  const rows=b.items.map(it=>`<tr style="border-top:1px solid #f3f4f6">
    <td style="padding:4px 8px;font-family:monospace;font-size:11px;word-break:break-all">${esc(it.name)}</td>
    <td style="padding:4px 8px;white-space:nowrap">${it.srcCode?esc(momoMoPlusSrcToMonth(it.srcCode)||it.srcCode):'<span style="color:#dc2626">判不出</span>'}</td>
    <td style="padding:4px 8px;font-weight:600;color:${statColor[it.status]||'#374151'};white-space:nowrap">${esc(it.status)}${it.detail?`<span style="color:#9ca3af;font-weight:400"> · ${esc(it.detail)}</span>`:''}</td>
  </tr>`).join('');
  const doneN=b.items.filter(it=>['成功','失敗','略過'].includes(it.status)).length;
  const okN=b.items.filter(it=>it.status==='成功').length, failN=b.items.filter(it=>it.status==='失敗').length;
  const progress = b.running ? `<div style="font-size:12px;color:#2563eb;margin-bottom:8px">⏳ 處理中… ${doneN}/${b.items.length}（成功 ${okN}／失敗 ${failN}）</div>` : '';
  const blockBanner = b.blocked ? `<div class="mm-banner mm-banner-err" style="margin-bottom:10px">⚠ ${esc(b.blockMsg)} → 請移除重複後重新選取（整批不處理，避免靜默覆蓋）。</div>` : '';
  let summary='';
  if(b.done){
  ...
  return `<div style="border:1px solid #eef0f2;border-radius:10px;padding:14px 16px">
    <div style="font-weight:700;margin-bottom:8px">批次上傳（${b.items.length} 檔）</div>
    ${blockBanner}${progress}
    <table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="color:#9ca3af;text-align:left"><th style="padding:4px 8px">檔名</th><th style="padding:4px 8px">期別（來源月）</th><th style="padding:4px 8px">狀態</th></tr></thead><tbody>${rows}</tbody></table>
    ${summary}
    <div style="display:flex;gap:10px;margin-top:12px">
      <button class="mm-btn-primary" onclick="momoMoPlusBatchRun('${shop}')" ${canRun?'':'disabled'}>▶ 開始處理（逐檔、全期別寫入）</button>
      <button onclick="momoMoPlusBatchClear('${shop}')" ${b.running?'disabled':''} style="padding:7px 14px;border-radius:7px;border:1px solid #e5e7eb;background:#fff;color:#6b7280;font-size:12px;cursor:pointer">清除清單</button>
    </div>
    <div style="font-size:11px;color:#9ca3af;margin-top:8px;line-height:1.6">逐檔依序處理（各自逐列驗證 / 運費判準 / 期別歸屬），任一檔失敗不中斷其他檔。全期別寫入＝比照單檔預設全勾；跨月尾巴以各檔月份為來源累加不覆蓋。</div>
  </div>`;
}
// ══════════════ MO+ 月對帳 UI（核對 momo 有沒有按對帳單收錢；逐項實際扣款，非甲乙費率估算）══════════════
let _momoMoPlusReconMonth={};    // shop → 'YYYY-MM'
let _momoMoPlusReconExpand={};   // shop → {A,B,C,D:bool}
// 可選月份：origins doc 來源月 ∪ 已存 recon doc 的月；新→舊
function momoMoPlusReconMonths(shop){
  const set=new Set();
  try{ Object.keys(momoListMoPlusOriginsDocs(shop)).forEach(src=>{ const m=momoMoPlusSrcToMonth(src); if(m) set.add(m); }); }catch(e){}
  try{ const pfx='ec_momo_reconcile|'+shop+'|';
    const scan=o=>{ if(o) Object.keys(o).forEach(k=>{ if(k.indexOf(pfx)===0) set.add(k.slice(pfx.length)); }); };
    scan(typeof Store!=='undefined'&&Store._profitMem); scan(typeof Store!=='undefined'&&Store._mem);
    for(let i=0;i<localStorage.length;i++){ const k=localStorage.key(i); if(k&&k.indexOf(pfx)===0) set.add(k.slice(pfx.length)); }
  }catch(e){}
  return [...set].filter(Boolean).sort().reverse();
}
function momoMoPlusReconSetMonth(shop,m){ _momoMoPlusReconMonth[shop]=m; momoRenderMoPlusRecon(shop); }
// 預設展開（費用組成一打開就看得到逐項）：state 未設＝展開；toggle 從當前有效狀態翻轉。
function momoMoPlusReconToggle(shop,g){ const e=_momoMoPlusReconExpand[shop]=_momoMoPlusReconExpand[shop]||{}; const cur=(e[g]!==false); e[g]=!cur; momoRenderMoPlusRecon(shop); }
// ── 對帳差異兩級判定（使用者定案）：差額佔 A 貨款 < MOMO_RECON_WARN_PCT ＝「輕微差異」(黃、幾元折讓尾差)，≥ ＝「不符」(紅、可能漏抓)。
//   tol=0：任何非零差額都顯示金額+%，不掩蓋、不自動修正（對帳頁硬性規則）。cmpV==null＝尚未有對帳單值。
const MOMO_RECON_WARN_PCT=0.1;   // 可調門檻（%）
function momoReconDiffInfo(sysV, cmpV, denomA){
  if(cmpV==null||cmpV==='') return { level:'na', color:'#9ca3af', text:'—', diff:null, pct:null };
  const diff=Math.round((Number(sysV)-Number(cmpV))*100)/100;
  const pct=(denomA>0)?(Math.abs(diff)/denomA*100):null;
  if(diff===0) return { level:'ok', color:'#10b981', text:'✓ 相符', diff:0, pct:0 };
  const warn=(pct!=null && pct<MOMO_RECON_WARN_PCT);
  const amt=(diff>0?'+':'')+diff.toLocaleString('en-US');
  return { level:warn?'warn':'bad', color:warn?'#d97706':'#dc2626',
    text:(warn?'⚠ 輕微差異 ':'✗ 不符 ')+amt+(pct!=null?'（'+pct.toFixed(2)+'%）':''), diff, pct };
}
...
function momoRenderMoPlusRecon(shop){
  const c=document.getElementById('momo-sub-content-'+shop); if(!c) return;
  const months=momoMoPlusReconMonths(shop);
  if(!_momoMoPlusReconMonth[shop] || months.indexOf(_momoMoPlusReconMonth[shop])<0) _momoMoPlusReconMonth[shop]=months[0]||'';
  const month=_momoMoPlusReconMonth[shop];
  const monthSel=months.length?`<select class="mm-sel" onchange="momoMoPlusReconSetMonth('${shop}',this.value)">${months.map(m=>`<option${m===month?' selected':''}>${m}</option>`).join('')}</select>`:'<span style="color:#9ca3af">（無月份）</span>';
  const hdr=`<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap">
    <span style="font-size:16px;font-weight:800">月對帳</span>
    <span style="font-size:12px;color:#6b7280">${momoShopDisplay(shop)}｜核對 momo 有沒有按對帳單收錢（掃有無紅色即可）</span>
    <span style="margin-left:auto;font-size:13px;color:#374151">月份 ${monthSel}</span>
  </div>`;
  if(!month){ c.innerHTML=hdr+`<div class="empty"><div class="empty-icon">🧾</div><div class="empty-hint">尚無資料——請先到「訂單明細」上傳本月對帳明細（mo+…對帳明細_YYMM.xls）</div></div>`; return; }
  ...
  const g1=momoMoPlusReconGroupHTML(shop,'A 貨款','A',sys.A,sys.Atot,denomA,'',true)
    ...
    +momoMoPlusReconGroupHTML(shop,'C 代扣運費','C',sys.C,sys.Ctot,denomA,'−',true)
    +momoMoPlusReconGroupHTML(shop,'D 手續費','D',sys.D,sys.Dtot,denomA,'−',true);
  const idInfo=momoReconDiffInfo(sys.Atot+sys.Btot-sys.Ctot-sys.Dtot, sys.E, denomA);   // 恆等式：tol=0、如實顯示差額+%（兩級）
  const g1id=`<div style="padding:8px 10px;background:#f8fafc;border-radius:8px;font-size:13px;margin-top:2px">
    A ${momoMoney(sys.Atot)} + B ${momoMoney(sys.Btot)} − C ${momoMoney(sys.Ctot)} − D ${momoMoney(sys.Dtot)} = <b>${momoMoney(sys.E)}</b> 實際入帳
...
function momoRenderUpload(shop){
  ...
  const jiaRow=`<div class="mm-uprow">
    <div class="mm-uplbl">甲配運費 <span class="mm-code">C1202</span><span class="opt">選填·可多檔</span><div class="mm-hint">第三方 + 超商 · 上傳時程：每月 11 號</div></div>
  ...
  // 乙配運費（C1204 寄倉分攤，逐SKU）：只在乙配頁收。檔內無日期→需明指月份。
  //   ⚠ 月份下拉只在「有選檔」時顯示（沒檔不該有下拉）；預設＝最新「有對帳單」的月（不是最新可見月）。
  const showYi=(shop==='乙配');
  const yiMonths=showYi?momoVisibleMonths(shop):[];
  if(showYi && (!_momoUpYiMonth || yiMonths.indexOf(_momoUpYiMonth)<0)){
    const rec=momoLatestReconciledMonth('乙配');
  ...
  const yiRow=showYi?`<div class="mm-uprow">
      <div class="mm-uplbl">乙配運費 <span class="mm-code">C1204</span><span class="opt">選填</span><div class="mm-hint">寄倉分攤 出貨+回收（逐SKU）· 上傳時程：每月 11 號</div></div>
  ...
  const active=[];
  if(f.c1105) active.push('銷量(C1105)');
  if(f.jia&&f.jia.length) active.push('甲配運費(C1202)');
  if(f.yi) active.push('乙配運費(C1204)');
  if(f.s1103) active.push('排行榜(S1103)');
  const genHint = anyFile ? ('將更新：'+active.join('、')+'（各走各的路徑，只傳運費/排行榜不動銷量）') : ('請至少選一個檔案（C1105／C1202'+(showYi?'／C1204':'')+'／S1103）');
  c.innerHTML=`
    <div style="max-width:780px">
      <div class="mm-note" style="background:#f9fafb;border:1px solid #eef0f2;border-radius:8px;padding:10px 12px;margin-bottom:14px">
        C1105／C1202${showYi?'／C1204':''}／S1103 <b>至少擇一</b>上傳即可，各走各的更新路徑：C1105→銷量、C1202→甲配運費${showYi?'、C1204→乙配運費':''}、S1103→排行榜。<br>
        <span class="mm-muted">${showYi?'乙配運費（C1204）逐SKU寄倉分攤，選對月份後產生預覽會顯示筆數/總額、寫入前再確認。':'乙配運費請到乙配頁上傳。'}只傳運費／排行榜<b>不會動銷量</b>。C1202 可跨月多檔（依訂編日期自動落到各月）。退貨走月對帳頁對帳單。</span>
      </div>
      ${fileRow('c1105','訂單商品明細','C1105','any','依配送類型分流至甲配／乙配（更新銷量）· 上傳時程：每月 1、16 號')}
      ${jiaRow}
      ${yiRow}
      ${fileRow('s1103','銷售排行榜','S1103',false,'瀏覽量／成交率（可單獨上傳補期別）· 上傳時程：每週一')}
...
// 最新「有對帳單」的月份（C1204 寫入月份預設用；比最新可見月合理——C1204 跟對帳單配對）
function momoLatestReconciledMonth(shop){
  let latest=''; const pf='ec_momo_reconcile|'+shop+'|';
  const scan=st=>{ if(!st) return; Object.keys(st).forEach(k=>{ if(k.indexOf(pf)===0){ const m=k.slice(pf.length); if(m>latest) latest=m; } }); };
  try{ scan(Store._profitMem); scan(Store._mem);
    for(let i=0;i<localStorage.length;i++){ const k=localStorage.key(i); if(k&&k.indexOf(pf)===0){ const m=k.slice(pf.length); if(m>latest) latest=m; } }
  }catch(e){}
  return latest;
}
...
function momoUploadGenerate(shop){
  ...
  if(!_momoUpFiles.c1105 && !_momoUpFiles.s1103 && !_momoUpFiles.yi && !(_momoUpFiles.jia&&_momoUpFiles.jia.length)){ alert('請至少選 C1105（銷量）/ C1202（甲配運費）/ C1204（乙配運費）/ S1103（排行榜）其一'); return; }
  ...
  Promise.all([
    ...
    // 乙配運費 C1204：逐SKU寄倉分攤，掃「全部」材積變體分頁(出貨+回收)。獨立於 C1105（可只傳 C1204 回補某月）。
    let yi=null, yiInfo=null;
    if(yiwb){
      const feeSheets=yiwb.names.filter(n=>/寄倉分攤.*運費/.test(n));   // 12 種材積變體，不只出貨/回收一般
      if(!feeSheets.length) throw new Error('乙配運費檔(C1204)找不到任何「寄倉分攤…運費」分頁（實際分頁：'+yiwb.names.join('、')+'）');
      yi={}; const perSheet={};
      feeSheets.forEach(n=>{ const m=momoParseYiFreightSheet(yiwb.sheet(n)); const sub=Object.values(m).reduce((a,b)=>a+b,0); if(sub!==0) perSheet[n]=Math.round(sub); Object.keys(m).forEach(sku=>{ yi[sku]=(yi[sku]||0)+m[sku]; }); });
      const total=Object.values(yi).reduce((a,b)=>a+b,0), skuN=Object.keys(yi).length;
      if(!(total>0)) throw new Error('乙配運費檔(C1204)解析後總額為 0（掃了 '+feeSheets.length+' 個分頁都無資料）——請確認檔案內容或分頁欄位（品號/扣款費用）。');
      if(!_momoUpYiMonth) throw new Error('請先在「寫入月份」選 C1204 要寫入的月份（檔案內沒有日期，需明指）。');
      yiInfo={ month:_momoUpYiMonth, skuN, total:Math.round(total), perSheet };
    }
    // ── 甲配運費 C1202（可多檔、可跨月）：逐檔統計落點月份（供核對「哪個檔進哪個月」）＋合併訂編→運費 → 混合分攤 ──
    //   有同月 C1105 → 逐SKU精算；沒有（只傳 C1202 回補）→ 按期別已存銷量估算。各走各的、不需重傳 C1105。
    let jia=null, jiaResult=null, jiaFilesBreakdown=null;
    if(jiaWbs.length){
      const orderFreight={}; jiaFilesBreakdown=[];
      jiaWbs.forEach((wb,i)=>{
        const raw=momoParseUnsendJia(wb.firstSheet());
        const fm={};
        Object.keys(raw.orderFreight).forEach(o=>{
          const fee=raw.orderFreight[o];
          if(orderFreight[o]===undefined) orderFreight[o]=fee;   // 合併去重（不同物流商訂單不重疊）
          const per=momoOrderToPeriod(o), mo=per?per.slice(0,7):'?';
          fm[mo]=fm[mo]||{n:0,total:0}; fm[mo].n++; fm[mo].total+=(fee||0);
        });
        jiaFilesBreakdown.push({ name:(jiaFiles[i]&&jiaFiles[i].name)||('檔'+(i+1)), months:fm });
      });
      jiaResult=momoAllocateJiaFreightHybrid(orderFreight, c1105);
      jia={freight:jiaResult.freight};
    }
    // S1105（退貨）只在有 C1105 的完整上傳時解析；只傳運費/排行榜不需要
    let s1105=null, s1105Error=null;
    if(c1105 && s1105wb){ try{ s1105=momoParseS1105(s1105wb.firstSheet()); }catch(err){ s1105Error=(err&&err.message)||String(err); console.warn('[momo] S1105 解析失敗，已略過（退貨率不會更新）：',s1105Error); } }
    // 沒有 C1105 → 不動銷量：精簡 plan，只帶運費(C1202/C1204)與排行榜(S1103)各自的寫入資料（各走各的路徑）
    if(!c1105){
      _momoUpPlan={ noSales:true, shops:{}, overwrite:[], unknownChannel:[],
        badPeriod:(jiaResult?jiaResult.badPeriod.slice():[]),
        s1103, s1103Sanity, s1103Error, yiInfo, yiFreight:yi, yiMonth:yiInfo?yiInfo.month:'',
        jiaFreight:(jia&&jia.freight)||null, jiaMonths:jiaResult?jiaResult.months:null, jiaFiles:jiaFilesBreakdown,
        jiaUnmatchedOrders:jiaResult?jiaResult.unmatchedOrders:[] };
...
// 甲配運費 C1202 落點/模式預覽（逐月：精算 vs 估算、SKU數、運費總額；逐檔：哪個檔進哪個月）
function momoJiaPreviewHTML(P){
  ...
  return `<div style="border:1px solid #bfdbfe;background:#eff6ff;border-radius:8px;padding:10px 12px;margin-bottom:8px;font-size:12px;color:#1e40af;line-height:1.6">
    🚚 <b>甲配運費（C1202）→ 依訂單編號日期落點</b>（月份依訂編判定、非檔名）
    <div style="margin-top:6px;background:#fff;border:1px solid #dbeafe;border-radius:6px;overflow:auto">
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead><tr style="text-align:left;color:#1e40af"><th style="padding:3px 8px">月份</th><th style="padding:3px 8px">模式</th><th style="padding:3px 8px;text-align:right">SKU 數</th><th style="padding:3px 8px;text-align:right">運費總額</th><th style="padding:3px 8px">備註</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
    ${fragBanner}
...
function momoRenderUploadPreview(shop){
  ...
  // 甲配運費 C1202：落點月份 + 模式（🟢精算／🟡估算）+ 逐檔對照（both c1105 與 noSales 路徑共用）
  const jiaHtml=momoJiaPreviewHTML(P);
  // 乙配運費 C1204：顯示筆數/總額/目標月份/各分頁 → 「看得到解析結果」（不再傳了沒反應）
  let yiHtml='';
  if(P.yiInfo){ const yi=P.yiInfo;
    const sheetRows=Object.keys(yi.perSheet||{}).map(n=>`${n} $${(yi.perSheet[n]||0).toLocaleString()}`).join('、');
    yiHtml=`<div style="border:1px solid #bfdbfe;background:#eff6ff;border-radius:8px;padding:10px 12px;margin-bottom:8px;font-size:12px;color:#1e40af;line-height:1.7">
      🚚 <b>乙配運費（C1204）→ 寫入 ${yi.month||'<span style="color:#dc2626">(未選月份)</span>'}</b>：${yi.skuN} 個品號、總額 <b>$${yi.total.toLocaleString()}</b>（出貨+回收，掃 ${Object.keys(yi.perSheet||{}).length} 個有資料分頁）
      ${sheetRows?`<div style="font-size:11px;color:#3b82f6;margin-top:2px">分頁：${sheetRows}</div>`:''}
      <div style="font-size:11px;color:#2563eb;margin-top:2px">按「確認寫入」後會再跳一次「即將寫入 ${yi.month||'?'} 乙配運費」確認。半月於計算時按銷量拆（估算）。</div>
    </div>`;
  }
  // 沒有 C1105（只傳運費／排行榜）：不動銷量，只顯示各自的寫入摘要（各走各的路徑）
  if(P.noSales){
    const canWrite=!!(P.jiaFreight || (P.yiFreight&&P.yiMonth) || P.s1103);
    el.innerHTML=`
      ...
      <div class="mm-note" style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 12px;margin-bottom:10px;font-size:12px;color:#1e40af">
        沒有訂單明細（C1105）→ <b>不動銷量</b>，只更新下列各自的資料（各走各的寫入路徑）。
      </div>
      ${jiaHtml}${yiHtml}${s1103Html}${badHtml}
      ${canWrite?`<button onclick="momoUploadApply('${shop}')" style="padding:7px 18px;border-radius:7px;border:none;background:#10b981;color:#fff;font-size:13px;font-weight:600;cursor:pointer">確認寫入</button>`:'<div style="color:#ef4444;font-size:13px">沒有可寫入的內容</div>'}
  ...
  el.innerHTML=`
    ...
    <button onclick="momoUploadShowDryRun('${shop}')" style="margin-left:8px;padding:7px 14px;border-radius:7px;border:1px solid #c7d2fe;background:#fff;color:#5b5fcf;font-size:13px;cursor:pointer" title="不寫入，先看每期別 qty/rev 前→後、新/累加、變小標紅，對過再執行">🔎 Dry-run 明細（不寫入）</button>
    <button onclick="momoUploadCancel('${shop}')" style="margin-left:8px;padding:7px 14px;border-radius:7px;border:1px solid #e5e7eb;background:#fff;color:#6b7280;font-size:13px;cursor:pointer">取消</button>`;
}
function momoUploadApply(shop, allowedPeriods){
  ...
  // ⓪-b 乙配運費 C1204：獨立 freight key（月總逐SKU；半月在計算時按銷量拆）。寫入前明示月份確認（選錯月會寫到錯期別）。
  let yiMonthWrote='';
  if(P.yiFreight && P.yiMonth){
    const yiTotal=Math.round(Object.values(P.yiFreight).reduce((a,b)=>a+b,0));
    if(!confirm('即將寫入 '+P.yiMonth+' 乙配運費（C1204）\n\n'+Object.keys(P.yiFreight).length+' 個品號、總額 $'+yiTotal.toLocaleString()+'\n\n月份正確嗎？選錯月會寫到錯的期別。')) return;   // 取消→整批不寫
    momoSaveFreight('乙配', P.yiMonth, { month:P.yiMonth, yiSku:P.yiFreight, total:yiTotal, source:'C1204' });
    yiMonthWrote=P.yiMonth; momoClearFeeRateCache();
  }
  // ⓪-c 甲配運費 C1202：獨立 freight key（不寫 cell → 不受 qty compact 防呆影響，已對帳月份也能灌運費）。逐月標 source（精算/估算）。
  let freightMonths=[];
  if(P.jiaFreight){
    const byMonth={};   // month → {sku:{period:$}}
    Object.keys(P.jiaFreight).forEach(sku=>{ Object.keys(P.jiaFreight[sku]).forEach(period=>{ const mo=period.slice(0,7); byMonth[mo]=byMonth[mo]||{}; byMonth[mo][sku]=byMonth[mo][sku]||{}; byMonth[mo][sku][period]=P.jiaFreight[sku][period]; }); });
    Object.keys(byMonth).forEach(mo=>{ const mode=(P.jiaMonths&&P.jiaMonths[mo]&&P.jiaMonths[mo].mode)||'precise'; momoSaveFreight('甲配', mo, { month:mo, freight:byMonth[mo], source:mode==='est'?'C1202-est':'C1202' }); freightMonths.push(mo); });
    if(freightMonths.length) momoClearFeeRateCache();
  }
  // 沒有 C1105（只寫運費/排行榜）：收工，不碰銷量 cell
  if(P.noSales){
    _momoUpPlan=null; _momoUpFiles.c1105=null; _momoUpFiles.jia=[]; _momoUpFiles.s1103=null; _momoUpFiles.yi=null; _momoUpYiMonth='';
    const parts=[]; if(freightMonths.length) parts.push('甲配運費 '+freightMonths.join('、')); if(yiMonthWrote) parts.push('乙配運費 '+yiMonthWrote); if(s1103Month) parts.push('排行榜 '+s1103Month);
    if(typeof showToast==='function') showToast(parts.length?('已寫入 '+parts.join('　+　')+'（記得按 ☁ 同步雲端）'):'未寫入任何資料','success');
    momoRenderUpload(shop);
    return;
  }
...
function momoParseCostList(rows){
  ...
  const num=v=>parseFloat(String(v).replace(/,/g,''))||0;
  // 「可用庫存」為選填欄（T 欄；舊檔可能沒有）→ 獨立找、缺欄=-1，不塞 momoLocateCols 必要欄以免整份 throw。
  //   ⚠ 檔案另有「實際庫存」(E 欄)，這裡取的是「可用庫存」——不要抓錯。
  const iStock=(rows[headerIdx]||[]).map(c=>String(c).trim()).indexOf('可用庫存');
  const costByOrigin={}, stockByOrigin={};
  for(let i=headerIdx+1;i<rows.length;i++){
    const r=rows[i]; if(!r) continue;
    const origin=String(r[idx.origin]||'').trim(); if(!origin) continue;
    if(costByOrigin[origin]===undefined) costByOrigin[origin]=num(r[idx.cost]);   // 成本每列相同→取第一個
    // 可用庫存：同一原廠編號（品項條碼）跨列加總。本檔通常一條碼一列（見樣本 B67-03/-04…各自一列），
    //   加總是為了防「萬一同條碼分多列」時不會只拿到一列；唯一列時加總＝自身值，不影響。
    if(iStock>=0){ stockByOrigin[origin]=(stockByOrigin[origin]||0)+num(r[iStock]); }
  }
  return {costByOrigin, stockByOrigin, hasStock:iStock>=0};
}
...
function momoCostAddHistoryToMoPlusProducts(origin, cost, from, note){
  ...
  Object.keys(byShop).forEach(shop=>{ const set=new Set(byShop[shop]); const products=momoLoadProducts(shop); let ch=false;
    products.forEach(p=>{ if(!set.has(p.sku)) return; p.history=p.history||[];
      p.history.push({ ...np, kind:'cost-shared', note:'因共用成本表變動：原廠 '+origin+' 成本 '+(from==null?'—':from)+'→'+cost+(n>1?'（同時影響 '+n+' 個 MO+ 商品）':'')+(note?' · '+note:''), costOrigin:origin, costVal:cost });
      ch=true;
    });
    if(ch) momoSaveProducts(shop, products);
  });
  return n;
}
function momoCostInlineToggle(origin){ const oid=String(origin).replace(/[^A-Za-z0-9]/g,'_'); const el=document.getElementById('mm-cost-inline-'+oid); if(el){ el.style.display=(el.style.display==='none'?'block':'none'); if(el.style.display==='block'){ const c=document.getElementById('mm-cost-inline-c-'+oid); if(c) c.focus(); } } }
function momoMoPlusCostInlineSave(shop, sku, origin){
  const oid=String(origin).replace(/[^A-Za-z0-9]/g,'_');
  const cEl=document.getElementById('mm-cost-inline-c-'+oid), nEl=document.getElementById('mm-cost-inline-n-'+oid);
  ...
  if(typeof showToast==='function') showToast('已補 '+origin+'＝'+r.to+'（'+n+' 個 MO+ 商品已記歷程，記得同步）','success');
  const p=momoLoadProducts(shop).find(x=>x.sku===sku); if(p) momoRenderMoPlusBatchEditForm(shop, p);   // 就地更新 costLines＋涵蓋率＋歷程
  try{ if((_momoSub[shop]||'')==='batch') momoRenderBatchEditList(shop); }catch(e){}
}
// 缺成本一次補：列出所有「毛利依賴此表卻查無成本」的 distinct 原廠編號（含影響商品數）→ 就地補、不必逐商品點進去。
function momoMoPlusMissingOrigins(shop){
  const costMap=momoMoPlusCostMapCached(); const m={};
  momoLoadProducts(shop).forEach(p=>{ const os=(p.origins&&p.origins.length)?p.origins:(p.origin?[p.origin]:[]); os.forEach(o=>{ if(o && !(Number(costMap[o])>0)) m[o]=(m[o]||0)+1; }); });
  return Object.keys(m).sort().map(o=>({origin:o, n:m[o]}));
}
function momoRenderMissingCostPanel(shop){
  const list=momoMoPlusMissingOrigins(shop);
  if(!list.length) return `<div style="color:#10b981;font-size:12px;padding:8px 2px">🎉 目前沒有缺成本的原廠編號</div>`;
  const rows=list.map(x=>{ const oid=String(x.origin).replace(/[^A-Za-z0-9]/g,'_'); return `<div style="display:flex;gap:6px;align-items:center;padding:3px 0;font-size:12px">
    <span style="font-family:monospace;width:110px">${_momoEsc(x.origin)}</span><span style="color:#9ca3af;width:64px">${x.n} 商品</span>
    <input id="mm-mc-c-${oid}" type="number" placeholder="成本" style="width:70px;padding:2px 6px;border:1px solid #e5e7eb;border-radius:5px" onkeydown="if(event.key==='Enter')momoMissingCostSave('${shop}','${_momoEsc(x.origin)}')">
...
function momoMissingCostSave(shop, origin){
  ...
  try{ momoBumpMoPlusEpoch(); }catch{}
  if(typeof showToast==='function') showToast('已補 '+origin+'＝'+r.to+'（'+n+' 個 MO+ 商品已記歷程）','success');
  momoRenderBatchEdit(shop);   // 重繪：該原廠從清單消失 + 商品涵蓋率/badge 更新
}
...
function momoExportCostByOrigin(){
  const map=momoLoadCostByOrigin(); const keys=Object.keys(map||{});
  if(!keys.length){ alert('目前沒有成本對照表（ec_momo_cost_by_origin）資料可匯出'); return; }
  const esc=v=>{ const s=String(v==null?'':v); return /[",\n\r]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s; };
  const lines=['原廠編號,成本']; keys.sort().forEach(o=>lines.push(esc(o)+','+esc(map[o])));
  ...
  try{ const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob);
    a.download='MOMO_成本對照表_'+new Date().toISOString().slice(0,10)+'.csv'; a.click();
...
function momoLoadStockByOrigin(){ const k=_MOMO_STOCK_LS;
  ...
  try{ const l=localStorage.getItem(k); if(l) return JSON.parse(l); }catch{}
  return {}; }
function momoSaveStockByOrigin(obj){ const k=_MOMO_STOCK_LS;   // ⚠ 不 _markPending：stock 走 momo_stock collection 自動推，不進 app/profit 手動同步流程（避免被推去錯地方）
  try{ localStorage.setItem(k,JSON.stringify(obj)); }catch{}
  try{ if(typeof Store!=='undefined'&&Store._profitMem) Store._profitMem[k]=obj; }catch{}
  try{ if(typeof Store!=='undefined'&&Store._mem) Store._mem[k]=obj; }catch{} }
function momoPersistStockByOrigin(stockByOrigin){   // 覆蓋為最新快照（不累積）+ 記上傳日期
  const byOrigin={}; let n=0;
  Object.keys(stockByOrigin||{}).forEach(o=>{ if(!o) return; byOrigin[o]=Number(stockByOrigin[o])||0; n++; });
  const obj={ uploadedAt: Date.now(), byOrigin };
  momoSaveStockByOrigin(obj);
  return {count:n, uploadedAt:obj.uploadedAt};
}
// 上傳成功後自動推雲（帳號級快照 last-write-wins：最新上傳覆蓋，符合「庫存以最新為準」）。
//   ⚠ 走獨立 collection momo_stock（__cloudStock.setSnapshot），不進 app/profit 欄位——6500+ 原廠編號的 map 會逐 key 建索引、
//     撞 app/profit 的 40,000 索引項上限（S1103 前車之鑑）。cost 這批不碰（要不要推 cost 另評估索引影響）。
//   best-effort：失敗不擋、資料留本機、可重新上傳再試。
async function momoAutoPushStock(){
  if(!window.__cloudStock || typeof window.__cloudStock.setSnapshot!=='function') return;
  let v=null; try{ v=(Store._mem&&Store._mem[_MOMO_STOCK_LS]!==undefined)?Store._mem[_MOMO_STOCK_LS]:(Store._profitMem&&Store._profitMem[_MOMO_STOCK_LS]); }catch{}
  if(v==null) return;
  try{ await window.__cloudStock.setSnapshot(v); console.log('[momoAutoPushStock] ✓'); if(typeof showToast==='function') showToast('已自動同步庫存到雲端','success'); }
  catch(e){ console.warn('[momoAutoPushStock] ✗',e); if(typeof showToast==='function') showToast('庫存自動同步失敗，資料在本機、可重新上傳再試','warn'); }
}
  ...
  c.innerHTML=`
    <div style="max-width:840px">
      <div style="font-size:12px;color:#6b7280;background:#f9fafb;border:1px solid #eef0f2;border-radius:8px;padding:10px 12px;margin-bottom:14px;line-height:1.6">
        <b>兩份檔至少擇一</b>即可產生差異預覽（同時比對 <b>甲配 + 乙配</b>）：只傳莫筆克成本＝比成本差異；只傳 MOMO 商品資訊＝比品名／售價／上下架；兩份都傳＝完整比對。莫筆克成本＝各倉商品列表（品項條碼→原廠編號、銷售成本→成本、可用庫存→總表「自家庫存」欄，上傳後自動同步雲端）；MOMO商品資訊＝MOMO後台商品主檔（倉別分通路）。套用後成本/價格走歷程（新增一筆、不覆蓋）。
      </div>
...
// ── 「倉租費」分頁已移除(v315)：手動月總從沒進計算(不分攤/不進毛利)、已被對帳單「寄倉倉租費(EC)」取代(Piece2起全額歸乙配)。
//    ec_momo_rent_records 舊資料刻意保留(localStorage+雲端 field 都不動、只移 UI 與同步白名單)，日後要找得回來。per-SKU 精算待 C1212。
...
// 從 2026-01（MOMO_FIRST_PERIOD）到指定月的完整連續月序（趨勢圖用；不再只取近 6 月）。
function momoOvMonthsRange(endMo){
  const arr=[]; let [y,m]=MOMO_FIRST_PERIOD.split('-').map(Number);
  const [ey,em]=endMo.split('-').map(Number);
  while(y<ey || (y===ey && m<=em)){ arr.push(y+'-'+String(m).padStart(2,'0')); m++; if(m>12){m=1;y++;} }
  return arr;
}
// 各賣場銷量 Top 10（依本期銷量 qty 排序，取前 10；各賣場各自算，品項不同故分圖）。
...
function momoRenderSummary(){
  ...
  const visKeys=new Set(momoAllPeriods('甲配').filter(k=>k.slice(0,7)>=MOMO_FIRST_PERIOD));
  ...
  const hasPrev=prevT.hasData, prevLbl=prevKey?momoPeriodLabel(prevKey):'上期';
  const rate=T.activeTotal>0?T.soldActive/T.activeTotal*100:null;
  const prevRate=(hasPrev&&prevT.activeTotal>0)?prevT.soldActive/prevT.activeTotal*100:null;
...
function momoOvBuildCharts(period, perShop){
  ...
  // 1. 營收+淨利趨勢
  mk('momo-ov-trend',{type:'line',data:{labels,datasets:[
    {label:'營收',data:revArr,borderColor:'#5b5fcf',backgroundColor:'#5b5fcf',tension:.3},
