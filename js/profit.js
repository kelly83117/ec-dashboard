...
function momoRenderMoPlusRecon(shop){
  ...
  // ⚠ 「② 系統計算 vs 對帳單」比對區塊 + 對帳單 PDF 上傳/解析 + 手動 B/F/I 三欄已移除（Vanessa 從未填過、無使用者）。
  //   保留 Vanessa 要的「每月費用組成」：費用組成四組（A/B/C/D 全線目含 0 值 + 佔 A%）與 A+B−C−D=E 恆等式檢核。
  //   已提領（sys.withdrawn）原只服務②的總額驗算 → 隨②移除、不再顯示（解析 momoParseMoPlusWithdrawn 仍在、無害，見回報）。
  c.innerHTML=hdr
    +`<div style="font-size:14px;font-weight:700;margin:4px 0 6px">費用組成（本月實際被扣了哪些、各多少）</div>`+g1+g1id;
}
// （momoMoPlusReconSavePdf / momoMoPlusReconPdfFile 已移除——對帳單 PDF 上傳/手動輸入/② 比對整塊移除，Vanessa 從未使用。）
...
Object.assign(window, {
  ...
  momoRenderMoPlusRecon,momoMoPlusReconSetMonth,momoMoPlusReconToggle,
