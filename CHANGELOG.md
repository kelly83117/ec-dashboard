...
- **月結表畫面改版**（同一支 PR；年度總表、評分表不動；公式／`_kpiGroupTotals`／fieldMerge 攤提一行未改）：
  - 總覽：填寫進度鈕（`_kpiFillSlots` 單一算法）、四格大數字（全通路純利／營收／訂單數／客單價，較上月沿用 cmpOk 守衛）、
    「營收成本結構」瀑布圖（每條：金額 · 佔營收 %）（類別對照 `KPI_WF_MAP`、「其他」用差額，console 印各類金額）、
    營收組成＋本月重點、各通路表（可展開看各店，唯讀）。原本逐欄可編輯的明細表移除。
  - 填寫模式：左側通路清單＋鍵盤說明；每格直接是輸入框（空格虛線框、「暫」／「未完成」）、合併欄位領頭格填總額、
    =公式與欄位備註保留；Enter／↓／Tab 移格、Enter 或失焦即存；Excel 多格貼上一次原子寫入；
    底部顯示最後編輯者與「下一個通路」。
  - 年／月兩個下拉合併成一個（2026/6、2026/5…），總覽與填寫模式共用；有資料的月份一定選得到。
  - 移除「清空此月份」（連同 deleteKpiRow）；要清一格＝清空後按 Enter。
  - 賣場調整：拿掉 MOMO 的 mo+2號店(露營館)（1–6 月全為 0，雲端資料保留不刪）；新增 PChome、FriDay、博客來三個通路（欄位同酷澎）；
    店名顯示改為 甲配／乙配／MO+好麻吉／MO+森之旅（KPI_SHOP_LABELS，只改顯示、app/kpi 的鍵不動）。
  - 填寫模式可讀性：表頭／店名／數字／唯讀計算欄／小計分層、空格淡黃底、鍵盤操作改成橫列按鍵說明。
  - 填寫模式每個賣場最左加「客單價」唯讀欄（實際營收 ÷ 訂單數；MOMO 用營收−退貨、蝦皮沿用原 aov 公式）；鍵盤說明改成「填寫模式」旁的 ? tooltip。
  - 新通路（PChome／FriDay／博客來）2026/8 起才列入填寫進度（之前月份顯示「不列入」）；總覽蝦皮展開不再列共同費用。
  - 月份選單：2025/1 起到下個月；2025 之前只列有資料的月份。字型比照淨利表 MOMO（字級、字重、顏色）。
