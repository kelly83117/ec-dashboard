...
- **月結表畫面改版**（同一支 PR；年度總表、評分表不動；公式／`_kpiGroupTotals`／fieldMerge 攤提一行未改）：
  ...
  - 填寫模式「⬇ 下載 Excel」：當月總表＋各通路一張，數字存數值（金額 $#,##0、純利率 0.00%）。
  - 金額一律 $（拿掉 NT）、較上月改 ▲▼；進度鈕改「填寫進度 a / b」；MOMO「應收帳款11日」顯示改「應收帳款」（欄位鍵不變）。
  - 1500px 以下填寫模式的通路清單改成表格上方一排分頁（讓寬度給表格）。
  - 總覽：營收組成改圓餅圖（Chart.js）；本月重點移到月份選單下、四張大卡上；全通路純利卡改同其他三張配色。
