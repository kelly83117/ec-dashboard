...
1. 先確認 **TEST_NOWRITE 防護碼**已經貼進 `js/firebase.js`（用 `ec-nowrite-on` 手動插入）。
   ⚠ 這段防護碼**刻意不存在於 repo 裡**：`grep TEST_NOWRITE` 零命中是**正確狀態**，不是異常，
   不要因為搜不到就以為它被移除了、或去補一套測試環境切換。
2. 在瀏覽器 F12 Console 看到紅字
   **「[TEST_NOWRITE] 已停用雲端寫入：13 物件 / N 方法」**，才能放心測。
   **判斷標準是「物件數對不對」：必須是 13**（2026-08-12 實測）。
   方法數 N 與下方 table 的條數**以工具實際印出的為準**，這份文件刻意不寫死——沒有實測依據。
3. 沒看到這行紅字、或**物件數不是 13** → **絕對不要繼續操作，立刻關閉分頁**。
   那代表有雲端物件沒被掃到，也就是有一條沒被保護的寫入路徑直通公司正式 Firestore。

⚠ **物件數會隨著新增雲端物件而變**（曾經是 4 物件 / 9 方法，早就過期）。不要當常數背，
每次改動 `js/firebase.js` 的雲端層都要當場重數並更新這裡：

```bash
grep -nE "^\s*(window\.)?__cloud[A-Za-z0-9_]*\s*=" js/firebase.js
```

🔴 字元集一定要用 `[A-Za-z0-9_]`，**不能**寫成 `[A-Za-z_]` —— 舊寫法吃不到數字，會漏掉
`__cloudS1103`，這正是「12 個物件」這個錯誤數字的來源。
**以工具實際印出的為準**：對不上先停手查是哪個物件沒被包到，不要直接改數字了事。

⚠ **已知問題（2026-08-12 發現，尚未修）**：防護碼的讀取白名單是**完全字串比對**，
只認 `getDoc` / `subscribe` 兩個名字。而 `__cloudInsightByShop` 的讀取方法叫
`getDocForShop` / `subscribeShop`，名字對不上 → **掛防護碼做本機測試時，洞察表的
per-shop 資料會讀不出來，而且不會報錯**。看到洞察表在本機是空的，先想到這條，
不要以為是自己改壞了。（正式站不受影響，防護碼只在本機手動插入。）
